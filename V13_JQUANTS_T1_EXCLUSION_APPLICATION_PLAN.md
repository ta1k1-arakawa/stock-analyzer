...
```text
STATUS=IMPLEMENTED_SYNTHETICALLY_AWAITING_GPT_EXACT_SHA_REVIEW
...
All repository/branch/HEAD/remote/clean/blob/environment/authority and path-topology checks precede source content read. The source opens once. On a nonempty first byte, the operation is consumed. A no-overwrite receipt is published and durably flushed using the previously reviewed Windows write-through and `FlushFileBuffers` primitives before any remainder byte is read. Receipt publication failure after first byte is a terminal post-boundary failure; no automatic retry or second source read exists. Pre-boundary failures leave the fresh authorization unconsumed. If a state or receipt already exists, the operation stops at pre-gate and reuse is prohibited pending adjudication.
