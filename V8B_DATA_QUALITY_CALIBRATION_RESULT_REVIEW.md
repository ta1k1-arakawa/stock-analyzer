...
execution_model=run_data_quality_calibration() (src/v8b_data_quality_calibration.py) is a pure core: it validates the VALID result state -- observed windows, all 30 candidates, synthetic bases/scenarios, and manifest/provenance -- against V8B_DATA_QUALITY_CALIBRATION_PREREGISTRATION_DRAFT.md §13's R0-R9 invariants before constructing and RETURNING the result artifact. The pure core itself does not persist/write any file; the production CLI (scripts/run_v8b_data_quality_calibration.py) serializes the returned result to stdout, and capture/persistence of that output into the artifact later referenced as attempt 2 occurred externally, outside the pure core.
production_core_pre_return_semantic_validation_attempt_2=PERFORMED_INTERNALLY (see execution_model above; validation happens once, before the core returns its result -- not at, or after, persistence, since the core does not persist)
FULL_SOURCE_BOUND_SEMANTIC_VALIDATION_POST_PERSISTENCE=NOT_REEXECUTED (this review, and the adjudication record it relies on, did not independently re-execute a second full source-bound semantic validation pass against the externally captured attempt-2 artifact after the pure core's own pre-return validation; the check performed on the already-captured artifact was self-hash/integrity and trusted commit/attempt-id binding only)
FULL_SOURCE_BOUND_SEMANTIC_VALIDATION_POST_PERSISTENCE_IS_A_FAILURE=false (this reflects the actual verification design -- the pure core validates once, before returning its result, against source; the post-capture check confirms the externally captured artifact was not altered afterward -- not a gap or defect)
```
