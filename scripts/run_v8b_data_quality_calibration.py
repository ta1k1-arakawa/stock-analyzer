#!/usr/bin/env python3
"""Gated, fixed-root-only CLI for the V8B production calibration execution
adapter.
...
``--confirm TOKEN --implementation-git-commit COMMIT --calibration-attempt-id ID``
    Human-gated production execution against the single fixed cache root
    (``src/v8b_data_quality_calibration_execution.py``'s ``V5B_CACHE_ROOT``).
...
from src.v8b_data_quality_calibration_execution import (  # noqa: E402
    V8BCalibrationExecutionBlocked,
    _canonical_execution_status,
    canonical_json_bytes,
    run_production_v8b_data_quality_calibration,
    run_static_check,
)

STATIC_SUCCESS_MESSAGE = "V8B_DATA_QUALITY_CALIBRATION_EXECUTION_STATIC_PASS"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Gated, fixed-root-only CLI for the V8B production calibration execution adapter.",
    )
    ...
    parser.add_argument(
        ...
        help="Caller-supplied calibration attempt identifier. Required with --confirm.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.static_check:
        if (
            ...
            or args.implementation_git_commit is not None
            or args.calibration_attempt_id is not None
        ):
            print("STATIC_CHECK_TAKES_NO_OTHER_ARGUMENTS", file=sys.stderr)
            return 2
        try:
            run_static_check()
        except V8BCalibrationExecutionBlocked as error:
    ...
    if not args.implementation_git_commit:
        ...
        return 2
    if not args.calibration_attempt_id:
        print("CALIBRATION_ATTEMPT_ID_REQUIRED", file=sys.stderr)
        return 2

    try:
        result = run_production_v8b_data_quality_calibration(
            confirmation=args.confirm,
            implementation_git_commit=args.implementation_git_commit,
            calibration_attempt_id=args.calibration_attempt_id,
        )
    except V8BCalibrationExecutionBlocked as error:
        payload = error.result if error.result is not None else _canonical_execution_status(error.detail)
