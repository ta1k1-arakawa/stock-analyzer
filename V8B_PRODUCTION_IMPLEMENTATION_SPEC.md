...
| `src/v8b_trust_pin_creation.py` | C | The production-gated `CREATE_V8B_TRUSTED_ALLOCATION_PIN` boundary (§12): obtains its verification summary only from the real `resolve_and_verify_t1b_allocation_artifact` resolver, requires `HUMAN_AUTHORIZATION_TO_PIN_VERIFIED_T1B_ALLOCATION` bound to the exact verified artifact hash, and durably one-shot-consumes that gate before writing the pin. Does **not** depend on `INDEPENDENT_TRUST_PIN_REVIEW` -- per the frozen §12 sequence, that review is strictly *downstream* of this module's own write, and is instead verified at `T1B` acquisition/artifact-verification time (`src/v8b_historical_acquisition.py`, `src/v8b_acquisition_artifact_verification.py`; corrected in remediation round 6, finding HIGH-1). Not executed by this phase. Added in remediation round 5 (finding HIGH-2). |
...
## 11. Remediation round 6 (`FINAL_REPEAT_INDEPENDENT_V8B_PRODUCTION_IMPLEMENTATION_REVIEW`: CRITICAL=0, HIGH=2, MEDIUM=3)

```text
high_findings_fixed=2
medium_findings_fixed=3
trust_pin_gate_sequence_corrected=true (HUMAN_AUTHORIZATION_TO_PIN_VERIFIED_T1B_ALLOCATION -> CREATE_V8B_TRUSTED_ALLOCATION_PIN -> INDEPENDENT_TRUST_PIN_REVIEW -> T1B_RAW_ACQUISITION_HUMAN_GATE)
trust_pin_review_bound_to_actual_published_pin_blob_and_commit=true
post_freeze_t2_facts_derived_from_v8_state_json_and_dedicated_stage_artifacts=true
one_shot_gate_ledger_derived_from_home_directory_not_checkout_path=true
public_error_reasons_audited_for_caller_controlled_suffixes=true
t2_acquisition_artifact_verification_independently_verifies_sealed_json=true
real_trust_pin_created=false
real_human_gate_consumed=false
real_t1b_acquisition=false
real_t2_acquisition=false
real_yahoo_requests=0
real_jpx_requests=0
research_opening_performed=false
layer_b_executed=false
frozen_final_candidate_selected=false
```

| Finding | Fix |
|---|---|
| HIGH-1: the frozen §12 gate sequence is `READ_ONLY_T1B_ALLOCATION_ARTIFACT_VERIFICATION` → `HUMAN_AUTHORIZATION_TO_PIN_VERIFIED_T1B_ALLOCATION` → `CREATE_V8B_TRUSTED_ALLOCATION_PIN` → `INDEPENDENT_TRUST_PIN_REVIEW` → `T1B_RAW_ACQUISITION_HUMAN_GATE`, but `src/v8b_trust_pin_creation.py` incorrectly required `INDEPENDENT_TRUST_PIN_REVIEW` *before* writing the pin -- backwards, since a review of the pin cannot precede the pin's own existence. | `src/v8b_trust_pin_creation.py` no longer depends on `INDEPENDENT_TRUST_PIN_REVIEW` at all: it durably, fail-closed, one-shot-consumes the new `HUMAN_AUTHORIZATION_TO_PIN_VERIFIED_T1B_ALLOCATION` gate (`src/v8b_human_gate_consumption.py`, new `GATE_PIN_VERIFIED_T1B_ALLOCATION`) strictly after allocation-verification/human-authorization checks and strictly before the pin write. `INDEPENDENT_TRUST_PIN_REVIEW` verification moved downstream to `T1B` acquisition (`src/v8b_historical_acquisition.py`) and `READ_ONLY_T1B_ACQUISITION_ARTIFACT_VERIFICATION` (`src/v8b_acquisition_artifact_verification.py`, which now re-establishes the complete `T1B` authority chain including this review, not merely the trust pin's own `authorization_status`). The review artifact's schema (`src.v8b_production_provenance`, bumped to `V8B_TRUST_PIN_INDEPENDENT_REVIEW_V2`) now mechanically binds to the actual published trust pin itself, not merely the allocation artifact: two new required fields, `reviewed_trust_pin_git_blob_sha` and `reviewed_trust_pin_git_commit`, are independently re-resolved against the real Git blob at both the reviewed commit (self-consistency) and the current verified HEAD (no drift/swap since review), and `reviewed_trust_pin_human_gate` is exact-value validated (not merely present). Neither a real trust pin nor a real review artifact was created -- every test is synthetic. |
| HIGH-2: the post-freeze `T2` preservation recheck treated the mere absence of an `open_for_*` API as sufficient proof that `T2` was never opened/exposed -- an API's absence proves nothing about whether the block was manually or otherwise opened outside that API. `layer_b_completed`/`frozen_final_candidate_established` were trusted purely as self-declared booleans on the future recheck artifact. | `src/v8b_t2_reuse_recheck.py` now combines the API-absence check with independent evidence read from `V8_STATE.json` (`T2.raw_data_acquired`, `T2.opened_for_research`, `T2.sealed_holdout_access_count`, `trust_anchor_pinning.block_assignments_exposed`) at the same verified head, using AND-for-safety/OR-for-risk semantics -- a field is only "safe" if *every* independent source agrees, and `V8_STATE.json`'s own `T2` record is genuinely authoritative cross-study evidence since `T2` is the literal same physical block reused from `V8` via the `OPTION_2` bridge. Two new dedicated, Git-tracked, exact-schema stage-completion approval artifacts (`src.v8b_production_provenance.read_and_verify_layer_b_completion` / `read_and_verify_frozen_final_candidate`, new `V8B_LAYER_B_COMPLETION_APPROVAL.json` / `V8B_FROZEN_FINAL_CANDIDATE_APPROVAL.json`) are now independently required to PASS in addition to -- not instead of -- the existing self-declared fields on `V8B_T2_REUSE_CONDITIONS_RECHECK.json`. None of these artifacts exist yet, so `T2` production acquisition continues to fail closed today by construction; no real Layer B run or frozen-final-candidate selection was performed. |
| MEDIUM-1: `CANONICAL_CONSUMPTION_STATE_ROOT` was derived from the checkout filesystem path (`Path(__file__).resolve().parents[1]`), so a second clone/worktree of this repository at a different path resolved to a different, empty receipt directory, silently defeating the one-shot guarantee for callers running from that second checkout. | `src/v8b_human_gate_consumption.py`'s canonical state root is now derived solely from the machine-local home directory (`Path.home()`, falling back to a fixed `/var/lib` path if home resolution fails) joined with a namespace fixed by a literal `REPOSITORY_IDENTITY` string (`ta1k1-arakawa/stock-analyzer`) -- never from this module's own checkout path. The receipt key additionally binds the repository identity, and receipts now carry a `"repository"` field. No public production API accepts a caller-supplied state root; there is still no deletion/reset API. A dedicated test loads two independent module instances from two different temp-directory "checkouts" (via `importlib.util.spec_from_file_location`) and proves they resolve to the identical canonical ledger and that a gate consumed via one blocks the other. |
| MEDIUM-2: `"V8B_BLOCK_ACQUISITION_PROHIBITED:" + str(block)` echoed the caller-controlled `block` parameter directly into a public error reason before it had been validated against the fixed `("T1B", "T2")` allow-list. | All three occurrences in `src/v8b_historical_acquisition.py` now raise the fixed literal `"V8B_BLOCK_ACQUISITION_PROHIBITED"` with no suffix. A full audit of every `Blocked(...)` call across the `v8b_*` production modules for caller-controlled/exception-text concatenation found no other instance of unbounded caller content reaching a public reason -- the remaining concatenations bind only to fixed internal literals (module parameter names, `BOUND_PRODUCTION_FILES` entries, other modules' own fixed reason vocabularies) or already-validated bounded enum values (e.g. `block` after the allow-list check, `key` over a fixed required-block-key tuple). New regression tests pass malicious-looking `block` values (a ticker, a private path, a URL, a date, an embedded newline) and prove none appear in the public reason. |
| MEDIUM-3: `READ_ONLY_T2_ACQUISITION_ARTIFACT_VERIFICATION` verified only the acquisition manifest's own self-reported `sealed`/`research_access_authorized`/access-counter fields -- it never independently re-read or verified the actual on-disk `SEALED.json` bundle file. | New `src.v8b_historical_acquisition.read_sealed_record` duplicate-key-safe parses and exact-schema/exact-value validates the real `SEALED.json` (`sealed == True`, `research_access_authorized == False`). `src/v8b_acquisition_artifact_verification.py`'s private checker now additionally requires the block bundle's top-level directory entries to be exactly the expected set (`acquisition_manifest.json` + `raw/`, plus `SEALED.json` only for `T2`) -- any missing/extra top-level entry BLOCKs -- and, for `T2`, calls `read_sealed_record` and propagates its fixed-literal failure reason. `T1B` explicitly BLOCKs (`T1B_BUNDLE_MUST_NOT_CONTAIN_SEALED_RECORD`) if a `SEALED.json` is ever present in its bundle. New tamper tests delete `SEALED.json`, set `sealed=false`, set `research_access_authorized=true`, duplicate a JSON key, write malformed JSON, add an unexpected top-level file/directory, and add `SEALED.json` to a `T1B` bundle -- all BLOCK. |

Full-boundary self-review (implementation review → T1B allocation → allocation verification → trust pin creation → INDEPENDENT_TRUST_PIN_REVIEW → T1B acquisition → T1B artifact verification → post-freeze T2 reuse check (V8_STATE.json evidence + stage-completion artifacts) → T2 acquisition → T2 artifact verification, including SEALED.json) repeated after this round found no further trust-root injection, stale-time evidence, un-recomputed claimed hash, caller-controlled authority value or public-reason leak, filesystem/exception privacy leak, caller override of a frozen rule, cross-block/cross-gate token reuse, research-opening path, retry/fallback/substitution, non-blob-bound reviewed dependency, checkout-path-local one-shot state, or self-declared-only stage-completion claim. No methodological ambiguity was found requiring `CHATGPT_DECISION_REQUIRED`.
