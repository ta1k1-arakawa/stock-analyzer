...
def train_ai_model(logger, config):
    ...
    # 目的変数の作成 
    future_days = train_config.get('future_days', 5)
    target_percent = train_config.get('target_percent', 2.0)
    ...
    logger.info(f"適合率 (Precision): {precision * 100:.2f} % (←「買い」予測の精度)")
    logger.info("----------------------------------")

    # 学習済みモデルの保存
    model_filename = train_config.get('model_save_path', 'stock_ai_model.pkl')
    try:
