...
def test_production_cli_phase_a_dispatches_to_real_module_function(monkeypatch, tmp_path, capsys):
    observed = []
    metadata = diagnostic.PhaseAMetadata(SimpleNamespace(), IMPLEMENTATION_SHA)
    monkeypatch.setattr(
        diagnostic,
...
def test_production_cli_diagnostic_uses_closed_safe_result(monkeypatch, tmp_path, capsys):
    ...
    calendar_path.write_text(json.dumps({"trading_dates": ["2018-01-01"]}), encoding="utf-8")
    ...
    result_code = runner.main(
        [*_argv(tmp_path, "--diagnostic"), "--calendar-json", str(calendar_path), "--authority-boundary-token", diagnostic.DIAGNOSTIC_BOUNDARY_TOKEN]
    )
    captured = capsys.readouterr()
    assert result_code == 0
