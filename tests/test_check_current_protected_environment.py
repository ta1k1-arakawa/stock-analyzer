...
def test_current_package_set_matches_the_resolved_27_package_lock() -> None:
    ...
    result = checker.check_live_package_set(authority["package_map"], _real_lock_text())
...
def test_package_drift_fails_closed() -> None:
    ...
    drifted = _real_lock_text().replace("lightgbm==4.6.0", "lightgbm==4.5.0")
    result = checker.check_live_package_set(packages, drifted)
    assert result["status"] == "FAIL"
