...
# `xlwt` is the same generator-only/dev fixture dependency already used by
# scripts/generate_synthetic_jpx_xls_fixture.py (never a production
# dependency); here it builds real, empirically-verified legacy `.xls` bytes
# so the real xlrd-based `_default_structural_inspector` can be exercised
# end-to-end rather than only through synthetic dict evidence.

def _xls_bytes(build: object) -> bytes:
...
def _all_cell_types_xls_bytes() -> bytes:
    def build(workbook: xlwt.Workbook) -> None:
        sheet = workbook.add_sheet("S")
        row = sheet.row(0)
        date_style = xlwt.XFStyle()
        date_style.num_format_str = "YYYY-MM-DD"
        row.set_cell_text(0, "text-value")
        row.set_cell_number(1, 3.5)
        row.set_cell_date(2, datetime.datetime(2020, 1, 1), date_style)
        row.set_cell_boolean(3, True)
        row.set_cell_error(4, 0x2A)
        row.set_cell_blank(5)
        # Column 6 is intentionally left unwritten: once the write below
        # extends ncols past it, xlrd reports it as XL_CELL_EMPTY.
        row.set_cell_text(7, "extend")

    return _xls_bytes(build)


class _FakeSheet:
    """A minimal stand-in for an xlrd Sheet, used only to exercise the two
    fail-closed branches (unrecognized visibility code, unrecognized cell
    type code) that real xlwt-generated bytes cannot produce, since xlwt can
    only emit the closed set of valid BIFF visibility/cell-type values."""

    def __init__(self, nrows: int, ncols: int, visibility: int, cell_type_code: int) -> None:
        self.nrows = nrows
        self.ncols = ncols
        self.visibility = visibility
        self._cell_type_code = cell_type_code

    def cell_type(self, _row: int, _col: int) -> int:
...
# against genuine xlwt-generated legacy `.xls` bytes wherever xlwt can
# express the case, and against a minimal fake Book/Sheet only for the two
# fail-closed branches (unrecognized visibility/cell-type code) that no
# valid BIFF file can contain. ------------------------------------------
...
def test_default_inspector_is_deterministic_across_repeated_calls() -> None:
    # design section 7: identical input bytes must produce byte-identical
    # safe-evidence output across repeated calls.
    raw = _all_cell_types_xls_bytes()
    first = probe._default_structural_inspector(raw)
    second = probe._default_structural_inspector(raw)
    assert json.dumps(first, sort_keys=True) == json.dumps(second, sort_keys=True)


def test_default_inspector_covers_all_seven_cell_type_buckets() -> None:
    # design section 5.10: exercises XL_CELL_TEXT/NUMBER/DATE/BOOLEAN/ERROR/
    # BLANK/EMPTY -- every entry in _CELL_TYPE_COUNT_KEYS -- via real xlrd
    # cell_type() classification of genuine xlwt-written cells.
    raw = _all_cell_types_xls_bytes()
    result = probe._default_structural_inspector(raw)
    assert result["status"] == "STRUCTURAL_FORMAT_CAPTURED"
...
def test_default_inspector_nrows_ncols_exact_with_blanks() -> None:
    # design section 5.9: row_count/column_count must equal xlrd's own
    # sheet.nrows/sheet.ncols exactly, including an unwritten-cell-implied
    # XL_CELL_EMPTY and an explicit XL_CELL_BLANK.
    def build(workbook: xlwt.Workbook) -> None:
        sheet = workbook.add_sheet("S")
        sheet.write(0, 0, "a")
        sheet.write(1, 0, "b")
        sheet.write(2, 0, "c")
        sheet.row(1).set_cell_blank(1)
        sheet.write(2, 1, 5)

    raw = _xls_bytes(build)
    result = probe._default_structural_inspector(raw)
    dims = result["structural_dimensions"][0]
...
def test_default_inspector_visibility_zero_one_two_round_trip() -> None:
    # design section 5.2: xlrd Sheet.visibility 0/1/2 must map exactly to
    # VISIBLE/HIDDEN/VERY_HIDDEN, via a real BOUNDSHEET visibility byte
    # (direct `worksheet.visibility = N` assignment on the xlwt Worksheet --
    # not xlwt's unrelated "sheet_visible"/active-tab selection state).
    def build(workbook: xlwt.Workbook) -> None:
        visible = workbook.add_sheet("Visible")
        visible.write(0, 0, "x")
        hidden = workbook.add_sheet("Hidden")
        hidden.write(0, 0, "x")
        hidden.visibility = 1
        very_hidden = workbook.add_sheet("VeryHidden")
        very_hidden.write(0, 0, "x")
