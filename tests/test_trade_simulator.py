...
class TestTradeSimulator:
 ...
 def test_gap_stop_uses_open_not_stop_price(self):
  df=frame([
      ...
      {"Open":100,"Low":96,"Close":100},
      {"Open":90,"Low":89,"Close":90},
  ])
  x=simulate_execution(df,0,2,5,stop_slippage_percent=1)
  assert x.exit_reason == "STOP"
  assert x.exit_price == pytest.approx(89.1)


 def test_open_equal_to_stop_uses_open(self):
  df=frame([
      {"Open":100,"Low":99,"Close":100},
      {"Open":100,"Low":96,"Close":100},
      {"Open":95,"Low":94,"Close":95},
  ])
  x=simulate_execution(df,0,2,5,stop_slippage_percent=1)
  assert x.exit_reason == "STOP"
  assert x.exit_price == pytest.approx(94.05)
 ...
 def test_low_above_stop_uses_time_exit(self):
  df=frame([
      {"Open":100,"Low":99,"Close":100},
      {"Open":100,"Low":96,"Close":100},
      {"Open":96,"Low":95.01,"Close":102},
  ])
  x=simulate_execution(df,0,2,5,stop_slippage_percent=1)
  assert x.exit_reason == "TIME"
  assert x.exit_price == pytest.approx(102)


 def test_zero_stop_slippage_fills_at_gap_open(self):
  df=frame([
      {"Open":100,"Low":99,"Close":100},
      {"Open":100,"Low":96,"Close":100},
      {"Open":90,"Low":89,"Close":90},
  ])
  x=simulate_execution(df,0,2,5,stop_slippage_percent=0)
  assert x.exit_reason == "STOP"
  assert x.exit_price == pytest.approx(90)
 ...
 def test_label_matches_realized_return_with_commission(self):
  df=frame([
      {"Open":100,"Low":99,"Close":100},
      {"Open":100,"Low":96,"Close":100},
      {"Open":105,"Low":100,"Close":110},
  ])
  execution=simulate_execution(df,0,2,5,commission_percent=1)
  labelled=create_target_variable(df,2,8,0,0,5,0,1)
  assert execution.exit_reason == "TIME"
  assert execution.return_percent == pytest.approx(7.9)
  assert labelled.iloc[0].Target == int(execution.return_percent >= 8)
