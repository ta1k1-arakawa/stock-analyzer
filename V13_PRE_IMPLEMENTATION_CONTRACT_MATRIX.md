...
| Stage A/B/C and transforms | Design: staged population | `build_rank_population`, `build_rank_population_audit` | two eligible sectors / independent pre/post Stage-B identity audit and rank-eligible identity | in-memory only; Q1, Q4, Q11 |
| Raw Stage-A features and target | Design: features/timing | `stage_a_from_raw`, `base_target` | valid ticker-observation windows / missing future value undefined | no calendar-contiguity gate or imputation; Q1, Q11 |
...
| Complete production path | Issue #31 HIGH_1; Issue #33 HIGH_1/MEDIUM_1 | `run_synthetic_feasibility` | frozen-seed selected-500 → manifest-derived 12-code raw Stage A/B/C → labels → monthly fits → all rankings → base/stress → metrics → A-P/Q | synthetic-only; Q1–Q12 |
