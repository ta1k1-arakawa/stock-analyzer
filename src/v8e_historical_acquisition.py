"""V8E raw-acquisition transport catch path and fixed production entrypoints.
...
in ``src.v8e_acquisition_engine`` is the sole raw-acquisition gate-consuming
path.
"""
...
class V8EAcquisitionBlocked(RuntimeError):
    def __init__(self, reason: str) -> None:
        super().__init__(reason)
        self.reason = reason


def execute_raw_acquisition_transport(*, stage: str, request_factory: Callable[[int], V8ERequestPlan],
    ...
    if stage not in ACQUISITION_STAGES:
        raise V8EAcquisitionBlocked("V8E_ACQUISITION_STAGE_INVALID")
    try:
        ...
        raise V8EAcquisitionBlocked(error.reason) from error
...
def execute_t1c_raw_acquisition_production(
    *, human_authorization_identity: str, partition_manifest_path: str | Path,
    ...
    from src.v8e_acquisition_engine import V8EAcquisitionEngineBlocked, _execute_fixed_production_acquisition
    ...
    except V8EAcquisitionEngineBlocked as error:
        raise V8EAcquisitionBlocked(error.reason) from error
