"""V8E production provenance: exact-blob-bound freeze/reviewed-implementation
verification.

`V8E_DQ_EVIDENCE_SUCCESSOR_DESIGN_DRAFT.md`, `V8E_DESIGN_FREEZE_
...
(`src.v8e_git_provenance.resolve_verified_v8e_production_git_commit`'s
...
This subtask (`V8E_PROD_HIGH_1A_REVIEWED_IMPLEMENTATION_BINDING`)
implements exactly:
...
- reviewed-implementation binding: a caller-supplied arbitrary 40-hex Git
  commit SHA is never, by itself, sufficient evidence that a V8E production
  ...
  `V8E_PRODUCTION_IMPLEMENTATION_REVIEW.json` artifact at a verified V8E
...
The `V8E_PRODUCTION_IMPLEMENTATION_REVIEW.json` artifact does not exist in
...
follow-up subtask, `V8E_PROD_HIGH_1B`).
...
EXPECTED_V8E_FROZEN_DESIGN_COMMIT = "6f672404b93a1003253915196dd635ca76fd2be1"
EXPECTED_V8E_DESIGN_DRAFT_BLOB_AT_FROZEN_COMMIT = "dac32f9e97d1ae2b90eb8b0820914e3845d0fa26"
EXPECTED_V8E_DESIGN_FREEZE_APPROVAL_BLOB = "651bc1a1cac122f8d36e6c19960db56161114f46"

DESIGN_DRAFT_GIT_PATH = "V8E_DQ_EVIDENCE_SUCCESSOR_DESIGN_DRAFT.md"
DESIGN_FREEZE_APPROVAL_GIT_PATH = "V8E_DESIGN_FREEZE_APPROVAL.json"

# --- Future INDEPENDENT_V8E_PRODUCTION_IMPLEMENTATION_REVIEW binding -------

IMPLEMENTATION_REVIEW_GIT_PATH = "V8E_PRODUCTION_IMPLEMENTATION_REVIEW.json"
IMPLEMENTATION_REVIEW_SCHEMA_VERSION = "V8E_PRODUCTION_IMPLEMENTATION_REVIEW_V1"
...
# --- Original immutable V8 authority used by the fixed V8E T0 probe ---------
...
# Every production-relevant V8E source file that must be byte-for-byte
# identical between current verified HEAD and the exact commit
# INDEPENDENT_V8E_PRODUCTION_IMPLEMENTATION_REVIEW reviewed. A later
...
BOUND_PRODUCTION_FILES: tuple[str, ...] = (
    "src/v8e_git_provenance.py",
    "src/v8e_production_provenance.py",
    "src/v8e_transport.py",
    "src/v8e_readiness.py",
    "src/v8e_historical_acquisition.py",
    "src/v8e_audit.py",
    "src/v8e_human_gate_consumption.py",
    "src/v8e_authority_bridge.py",
    "src/v8e_readiness_audit_verification.py",
    "src/v8e_t2_point_of_use_preservation.py",
    "src/v8e_acquisition_engine.py",
)


class V8EProductionProvenanceBlocked(RuntimeError):
...
def require_git_commit(value: object, reason: str = "GIT_COMMIT_INVALID") -> str:
    ...
    own ``V8EProductionProvenanceBlocked`` (not the generic-primitive
    ``V8BGitProvenanceBlocked``) so every caller's ``except
    V8EProductionProvenanceBlocked`` clause actually catches it."""
    try:
        return _require_git_commit_raw(value, reason)
    except V8BGitProvenanceBlocked as error:
        raise V8EProductionProvenanceBlocked(error.reason) from error
...
def _strict_json_object(raw: bytes, *, invalid_reason: str, duplicate_reason: str) -> dict[str, Any]:
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise V8EProductionProvenanceBlocked(duplicate_reason)
            result[key] = value
        return result
    ...
    try:
        ...
        raise V8EProductionProvenanceBlocked(invalid_reason) from error
    if not isinstance(parsed, dict):
        raise V8EProductionProvenanceBlocked(invalid_reason)
    return parsed
...
def _wrap_git_provenance_error(
    error: V8EGitProvenanceBlocked, missing_reason: str | None = None
) -> V8EProductionProvenanceBlocked:
    if missing_reason is not None and error.reason in _GIT_OBJECT_MISSING_REASONS:
        return V8EProductionProvenanceBlocked(missing_reason)
    return V8EProductionProvenanceBlocked(error.reason)
...
def verify_frozen_design_object(repository_root) -> None:
    """Verify the frozen `V8E_DQ_EVIDENCE_SUCCESSOR_DESIGN_DRAFT.md`
    object *at the frozen design commit itself* -- not at current HEAD --
    exactly matches the blob independently reviewed."""
    try:
        blob = resolve_git_blob(repository_root, EXPECTED_V8E_FROZEN_DESIGN_COMMIT, DESIGN_DRAFT_GIT_PATH)
    except V8EGitProvenanceBlocked as error:
        raise _wrap_git_provenance_error(error) from error
    if blob != EXPECTED_V8E_DESIGN_DRAFT_BLOB_AT_FROZEN_COMMIT:
        raise V8EProductionProvenanceBlocked("V8E_FROZEN_DESIGN_OBJECT_MUTATED")
...
def verify_design_freeze_approval_blob(repository_root, verified_head: str) -> str:
    """Verify `V8E_DESIGN_FREEZE_APPROVAL.json` at the current *verified*
    ...
    try:
        ...
        raise _wrap_git_provenance_error(error, "V8E_DESIGN_FREEZE_APPROVAL_MISSING") from error
    if blob != EXPECTED_V8E_DESIGN_FREEZE_APPROVAL_BLOB:
        raise V8EProductionProvenanceBlocked("V8E_DESIGN_FREEZE_APPROVAL_BLOB_MUTATED")
    return blob
...
def read_and_verify_v8_trusted_partition_anchor(repository_root, verified_head: str) -> dict[str, Any]:
    ...
    if blob != EXPECTED_V8_TRUSTED_PARTITION_BLOB_SHA:
        raise V8EProductionProvenanceBlocked("V8_TRUSTED_PARTITION_BLOB_MUTATED")
    try:
    ...
    if set(anchor) != set(TRUSTED_PARTITION_ANCHOR_FIELDS):
        raise V8EProductionProvenanceBlocked("TRUSTED_PARTITION_ANCHOR_SCHEMA_INVALID")
    if anchor["schema_version"] != TRUSTED_PARTITION_ANCHOR_SCHEMA_VERSION:
        raise V8EProductionProvenanceBlocked("TRUSTED_PARTITION_ANCHOR_SCHEMA_VERSION_MISMATCH")
    if anchor["design_commit"] != V8_DESIGN_COMMIT:
        raise V8EProductionProvenanceBlocked("TRUSTED_PARTITION_ANCHOR_DESIGN_COMMIT_MISMATCH")
    if anchor["authorization_status"] != "AUTHORIZED":
        raise V8EProductionProvenanceBlocked("TRUSTED_PARTITION_NOT_AUTHORIZED")
    if anchor["authorized_partition_manifest_sha256"] != EXPECTED_V8_PARTITION_MANIFEST_SHA256:
        raise V8EProductionProvenanceBlocked("TRUSTED_PARTITION_MANIFEST_SHA_MISMATCH")
    if anchor["authorized_partition_implementation_git_commit"] != EXPECTED_V8_PARTITION_IMPLEMENTATION_COMMIT:
        raise V8EProductionProvenanceBlocked("TRUSTED_PARTITION_IMPLEMENTATION_GIT_COMMIT_MISMATCH")
    return dict(anchor)


# ---------------------------------------------------------------------------
# Reviewed-implementation binding (future INDEPENDENT_V8E_PRODUCTION_
...
def verify_reviewed_implementation_binding(repository_root, verified_head: str) -> dict[str, Any]:
    """Bind production to the exact commit `INDEPENDENT_V8E_PRODUCTION_
    ...
    try:
        ...
        raise _wrap_git_provenance_error(error, "V8E_PRODUCTION_IMPLEMENTATION_REVIEW_MISSING") from error
    review = _strict_json_object(
        raw,
        invalid_reason="V8E_PRODUCTION_IMPLEMENTATION_REVIEW_INVALID_JSON",
        duplicate_reason="V8E_PRODUCTION_IMPLEMENTATION_REVIEW_DUPLICATE_KEY",
    )
    if set(review) != set(IMPLEMENTATION_REVIEW_FIELDS):
        raise V8EProductionProvenanceBlocked("V8E_PRODUCTION_IMPLEMENTATION_REVIEW_SCHEMA_INVALID")
    if review["schema_version"] != IMPLEMENTATION_REVIEW_SCHEMA_VERSION:
        raise V8EProductionProvenanceBlocked("V8E_PRODUCTION_IMPLEMENTATION_REVIEW_SCHEMA_VERSION_MISMATCH")
    if review["study"] != STUDY_NAME:
        raise V8EProductionProvenanceBlocked("V8E_PRODUCTION_IMPLEMENTATION_REVIEW_STUDY_MISMATCH")
    if review["artifact_role"] != IMPLEMENTATION_REVIEW_ARTIFACT_ROLE:
        raise V8EProductionProvenanceBlocked("V8E_PRODUCTION_IMPLEMENTATION_REVIEW_ARTIFACT_ROLE_MISMATCH")
    if review["review_result"] != "PASS":
        raise V8EProductionProvenanceBlocked("V8E_PRODUCTION_IMPLEMENTATION_REVIEW_NOT_PASS")
    if review["approval_status"] != "APPROVED":
        raise V8EProductionProvenanceBlocked("V8E_PRODUCTION_IMPLEMENTATION_REVIEW_NOT_APPROVED")

    reviewed_commit = require_git_commit(
        review["reviewed_implementation_git_commit"], "V8E_PRODUCTION_IMPLEMENTATION_REVIEW_COMMIT_INVALID"
    )
    ...
    for path in BOUND_PRODUCTION_FILES:
        try:
            ...
            raise _wrap_git_provenance_error(error, "V8E_BOUND_FILE_MISSING_AT_HEAD:" + path) from error
        try:
            blob_reviewed = resolve_git_blob(repository_root, reviewed_commit, path)
        except V8EGitProvenanceBlocked as error:
            raise _wrap_git_provenance_error(error, "V8E_BOUND_FILE_MISSING_AT_REVIEWED_COMMIT:" + path) from error
        if blob_head != blob_reviewed:
            raise V8EProductionProvenanceBlocked("V8E_REVIEWED_IMPLEMENTATION_BLOB_DRIFT:" + path)
...
__all__ = [
    ...
    "EXPECTED_V8E_DESIGN_DRAFT_BLOB_AT_FROZEN_COMMIT",
