...
def _required_manifest_sha256(manifest: Mapping[str,Any], key: str) -> str:
    """Return a provenance hash only when its manifest value is canonical SHA-256."""
    value=manifest.get(key)
    if not isinstance(value,str) or len(value)!=64 or any(char not in "0123456789abcdef" for char in value):
        raise ValueError(f"MANIFEST_{key.upper()}_INVALID")
    return value
