"""Independent read-only verification for V8E transport artifacts.

`V8E_DQ_EVIDENCE_SUCCESSOR_DESIGN_DRAFT.md` §8 (independent transport-
...
``gate_receipt_state_root`` (`src.v8e_human_gate_consumption.read_gate_
...
test_v8e_transport.py` with a fixed synthetic SHA) and carry no production
...
verified V8E Git HEAD -> frozen design verification -> freeze approval
verification -> `src.v8e_production_provenance.verify_reviewed_
implementation_binding` -- and require it to match the dossier's/
aggregate's claimed commit as the exact expectation. The real
`V8E_PRODUCTION_IMPLEMENTATION_REVIEW.json` does not exist in this
...
from src.v8e_transport import (
    ...
    validate_dq_evidence,
    V8ETransportBlocked,
)


class V8EAuditVerificationBlocked(RuntimeError):
...
def _read_json(path: str | Path) -> dict[str, Any]:
    try:
        ...
        raise V8EAuditVerificationBlocked("V8E_AUDIT_JSON_INVALID") from error
    if not isinstance(value, dict):
        raise V8EAuditVerificationBlocked("V8E_AUDIT_OBJECT_INVALID")
    return value
...
def _require_hex(value: object, length: int, reason: str) -> None:
    ...
    if not isinstance(value, str) or not pattern.fullmatch(value):
        raise V8EAuditVerificationBlocked(reason)


def _derive_origin_condition(evidence: object) -> str | None:
    if not isinstance(evidence, Mapping):
        raise V8EAuditVerificationBlocked("V8E_ORIGIN_EVIDENCE_INVALID")
    ...
    if set(evidence) != required or evidence.get("detector_source") != "ORIGIN_GUARD":
        raise V8EAuditVerificationBlocked("V8E_ORIGIN_EVIDENCE_SCHEMA_INVALID")
    ...
    if any(type(value) is not bool for value in booleans):
        raise V8EAuditVerificationBlocked("V8E_ORIGIN_EVIDENCE_BOOLEAN_INVALID")
    if evidence["context"] not in {"REDIRECT_TARGET", "INITIAL_OR_FINAL_RESPONSE"}:
        raise V8EAuditVerificationBlocked("V8E_ORIGIN_CONTEXT_INVALID")
    if (not evidence["input_is_string"] or not evidence["origin_parse_success"]) and any(booleans[2:]):
        raise V8EAuditVerificationBlocked("V8E_ORIGIN_DOWNSTREAM_FILL_INVALID")
    invalid = not all(booleans)
    if not invalid:
        raise V8EAuditVerificationBlocked("V8E_ORIGIN_FAILURE_WITH_VALID_ORIGIN")
...
def _derive_named_condition(named: object, evidence: object) -> tuple[str, bool]:
    if not isinstance(named, str):
        raise V8EAuditVerificationBlocked("V8E_NAMED_CONDITION_MISSING")
    if isinstance(evidence, Mapping) and evidence.get("detector_source") == "ORIGIN_GUARD":
        derived = _derive_origin_condition(evidence)
        if named != derived:
            raise V8EAuditVerificationBlocked("V8E_NAMED_CONDITION_MISMATCH")
        return derived, False
    if named == "SYMBOL_MISMATCH":
        if evidence != {"expected_symbol_binding": False}:
            raise V8EAuditVerificationBlocked("V8E_SYMBOL_EVIDENCE_INVALID")
        return named, False
    if named == "DATA_QUALITY_GATE_FAILURE":
        try:
            validate_dq_evidence(evidence)
        except V8ETransportBlocked as error:
            raise V8EAuditVerificationBlocked(error.reason) from error
        return named, False
    if named == "PARSER_SCHEMA_FAILURE":
        if isinstance(evidence, Mapping) and set(evidence) == {"parser_schema_valid", "canonical_collector_reason_code_or_family"}:
            if evidence["parser_schema_valid"] is not False:
                raise V8EAuditVerificationBlocked("V8E_PARSER_EVIDENCE_SCHEMA_INVALID")
            ...
            if classification != named or retryable:
                raise V8EAuditVerificationBlocked("V8E_PARSER_REASON_CLASSIFICATION_MISMATCH")
            return named, False
        ...
        if classification not in {"PARSER_SCHEMA_FAILURE", "RESPONSE_HOST_MISMATCH", "SYMBOL_MISMATCH"} or retryable:
            raise V8EAuditVerificationBlocked("V8E_PARSER_REASON_INVALID")
        if classification != named:
            raise V8EAuditVerificationBlocked("V8E_PARSER_REASON_CLASSIFICATION_MISMATCH")
        return named, False
    raise V8EAuditVerificationBlocked("V8E_NAMED_EVIDENCE_INVALID")
...
def _derive_transport_classification(record: Mapping[str, Any]) -> tuple[str, bool | None]:
    ...
    if classification == "SUCCESS":
        if record.get("retryable") is not None or record.get("concrete_exception_type") is not None:
            raise V8EAuditVerificationBlocked("V8E_SUCCESS_METADATA_INVALID")
    ...
    if named is not None:
        if record.get("concrete_exception_type") != "V8ENamedFailure":
            raise V8EAuditVerificationBlocked("V8E_NAMED_EXCEPTION_TYPE_INVALID")
    ...
    if evidence_source == "CANONICAL_COLLECTOR_REASON":
        if record.get("concrete_exception_type") != "V7YahooCollectorBlocked":
            raise V8EAuditVerificationBlocked("V8E_COLLECTOR_EXCEPTION_TYPE_INVALID")
        reason = evidence.get("canonical_collector_reason_code_or_family")
    ...
    if code is not None:
        if type(code) is not int:
            raise V8EAuditVerificationBlocked("V8E_HTTP_CODE_INVALID")
        if concrete != "HTTPError":
            raise V8EAuditVerificationBlocked("V8E_HTTP_EXCEPTION_TYPE_INVALID")
        if record.get("reason_type") is not None or record.get("errno") is not None:
            raise V8EAuditVerificationBlocked("V8E_HTTP_METADATA_INVALID")
    ...
    if observed_errno is not None and type(observed_errno) is not int:
        raise V8EAuditVerificationBlocked("V8E_ERRNO_INVALID")
    if concrete in {"TimeoutError", "socket.timeout"}:
        if reason_type != concrete or observed_errno is not None:
            raise V8EAuditVerificationBlocked("V8E_TIMEOUT_METADATA_INVALID")
    ...
    if concrete == "ConnectionResetError":
        if reason_type != concrete:
            raise V8EAuditVerificationBlocked("V8E_RESET_METADATA_INVALID")
        return "CONNECTION_RESET", True
    ...
    if concrete == "gaierror":
        if reason_type != concrete:
            raise V8EAuditVerificationBlocked("V8E_DNS_METADATA_INVALID")
        if observed_errno == socket.EAI_AGAIN:
            return "TEMPORARY_DNS_FAILURE", True
        return "UNKNOWN_FAIL_CLOSED_NONRETRYABLE", False
    if not isinstance(concrete, str) or reason_type != concrete:
        raise V8EAuditVerificationBlocked("V8E_UNKNOWN_METADATA_INVALID")
    return "UNKNOWN_FAIL_CLOSED_NONRETRYABLE", False
...
def verify_dossier(path: str | Path, *, gate_receipt_state_root: str | Path | None = None,
    ...
    if gate_receipt_state_root is None:
        raise V8EAuditVerificationBlocked("V8E_GATE_RECEIPT_STATE_ROOT_REQUIRED")
    dossier = _read_json(path)
    if set(dossier) != _DOSSIER_FIELDS:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_SCHEMA_INVALID")
    if dossier["schema_version"] != "V8E_PRIVATE_TRANSPORT_AUDIT_V1" or dossier["study"] != STUDY:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_STUDY_SCHEMA_INVALID")
    if dossier["frozen_design_commit"] != FROZEN_DESIGN_COMMIT:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_DESIGN_BINDING_MISMATCH")
    _require_hex(dossier["reviewed_production_implementation_commit"], 40, "V8E_DOSSIER_IMPLEMENTATION_SHA_INVALID")
    if expected_reviewed_implementation_commit is not None and dossier["reviewed_production_implementation_commit"] != expected_reviewed_implementation_commit:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_IMPLEMENTATION_BINDING_MISMATCH")
    if dossier["canonical_parser_classifier_commit"] != CANONICAL_PARSER_CLASSIFIER_COMMIT or dossier["canonical_parser_classifier_blob"] != CANONICAL_PARSER_CLASSIFIER_BLOB:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_CLASSIFIER_BINDING_MISMATCH")
    stage = dossier["logical_stage"]
    if stage not in ALL_STAGES or (expected_stage is not None and stage != expected_stage):
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_STAGE_INVALID")
    expected_gate = STAGE_GATE.get(stage)
    if expected_gate is None or dossier["human_gate"] != expected_gate:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_HUMAN_GATE_MISMATCH")
    _require_hex(dossier["gate_receipt_key_sha256"], 64, "V8E_DOSSIER_GATE_RECEIPT_KEY_INVALID")
    _require_hex(dossier["gate_receipt_bytes_sha256"], 64, "V8E_DOSSIER_GATE_RECEIPT_BYTES_HASH_INVALID")
    _require_hex(dossier["authorization_identity_sha256"], 64, "V8E_DOSSIER_AUTHORIZATION_IDENTITY_HASH_INVALID")
    ...
    try:
        receipt, receipt_bytes_sha256 = read_gate_consumption_receipt_with_bytes_hash(
            ...
            expected_gate=expected_gate, expected_v8e_frozen_design_commit=FROZEN_DESIGN_COMMIT,
        )
    except V8EHumanGateConsumptionBlocked as error:
        raise V8EAuditVerificationBlocked(error.reason) from error
    if receipt_bytes_sha256 != dossier["gate_receipt_bytes_sha256"]:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_GATE_RECEIPT_BYTES_MISMATCH")
    if receipt["authorization_identity_sha256"] != dossier["authorization_identity_sha256"]:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_GATE_RECEIPT_AUTHORIZATION_MISMATCH")
    if receipt["reviewed_production_implementation_commit"] != dossier["reviewed_production_implementation_commit"]:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_GATE_RECEIPT_IMPLEMENTATION_MISMATCH")
    if receipt["logical_stage"] != stage:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_GATE_RECEIPT_STAGE_MISMATCH")
    ...
    if dossier["logical_block"] != expected_block or type(dossier["logical_coordinate"]) is not int or dossier["logical_coordinate"] < 0:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_LOGICAL_BINDING_INVALID")
    if stage in READINESS_STAGES:
        if dossier["sentinel_indices"] != list(SENTINEL_INDICES) or dossier["window_start"] != SENTINEL_START or dossier["window_end_exclusive"] != SENTINEL_END_EXCLUSIVE:
            raise V8EAuditVerificationBlocked("V8E_DOSSIER_SENTINEL_BINDING_INVALID")
        if dossier["logical_coordinate"] not in SENTINEL_INDICES:
            raise V8EAuditVerificationBlocked("V8E_DOSSIER_SENTINEL_COORDINATE_INVALID")
    elif dossier["sentinel_indices"] != []:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_ACQUISITION_SENTINEL_INVALID")
    attempts = dossier["attempts"]
    if not isinstance(attempts, list) or not attempts or len(attempts) > MAXIMUM_ATTEMPTS:
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_ATTEMPTS_INVALID")
    if dossier["audit_artifact_self_hash"] != canonical_sha256({key: value for key, value in dossier.items() if key != "audit_artifact_self_hash"}):
        raise V8EAuditVerificationBlocked("V8E_DOSSIER_SELF_HASH_MISMATCH")
    ...
    for expected_attempt, record in enumerate(attempts, start=1):
        if not isinstance(record, Mapping) or set(record) != _ATTEMPT_FIELDS or record["attempt"] != expected_attempt:
            raise V8EAuditVerificationBlocked("V8E_DOSSIER_ATTEMPT_ORDER_INVALID")
        _require_hex(record["request_fingerprint"], 64, "V8E_DOSSIER_FINGERPRINT_INVALID")
        _require_hex(record["request_url_sha256"], 64, "V8E_DOSSIER_URL_HASH_INVALID")
...
def verify_aggregate(aggregate_path: str | Path, dossier_paths: Sequence[str | Path], *,
    ...
    if set(aggregate) != _AGGREGATE_FIELDS:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_SCHEMA_INVALID")
    if aggregate["schema_version"] != "V8E_PUBLIC_TRANSPORT_AGGREGATE_V1" or aggregate["study"] != STUDY:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_STUDY_SCHEMA_INVALID")
    if aggregate["aggregate_self_hash"] != canonical_sha256({key: value for key, value in aggregate.items() if key != "aggregate_self_hash"}):
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_SELF_HASH_MISMATCH")
    if aggregate["frozen_design_commit"] != FROZEN_DESIGN_COMMIT or aggregate["canonical_parser_classifier_commit"] != CANONICAL_PARSER_CLASSIFIER_COMMIT or aggregate["canonical_parser_classifier_blob"] != CANONICAL_PARSER_CLASSIFIER_BLOB:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_PROVENANCE_MISMATCH")
    if expected_stage is not None and aggregate["logical_stage"] != expected_stage:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_STAGE_MISMATCH")
    if aggregate["logical_stage"] not in ALL_STAGES:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_STAGE_INVALID")
    if aggregate["logical_block"] != ("T1C" if aggregate["logical_stage"].startswith("T1C") else "T2"):
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_BLOCK_INVALID")
    if aggregate["logical_stage"] in READINESS_STAGES:
        if aggregate["sentinel_indices"] != list(SENTINEL_INDICES) or aggregate["window_start"] != SENTINEL_START or aggregate["window_end_exclusive"] != SENTINEL_END_EXCLUSIVE:
            raise V8EAuditVerificationBlocked("V8E_AGGREGATE_SENTINEL_BINDING_INVALID")
    elif aggregate["sentinel_indices"] != []:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_ACQUISITION_SENTINEL_INVALID")
    _require_hex(aggregate["reviewed_production_implementation_commit"], 40, "V8E_AGGREGATE_IMPLEMENTATION_SHA_INVALID")
    if expected_reviewed_implementation_commit is not None and aggregate["reviewed_production_implementation_commit"] != expected_reviewed_implementation_commit:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_IMPLEMENTATION_MISMATCH")
    dossiers = [
        verify_dossier(
    ...
    if len(dossiers) != aggregate["audit_artifact_count"] or not dossiers:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_DOSSIER_COUNT_MISMATCH")
    if any(dossier["logical_block"] != aggregate["logical_block"] for dossier in dossiers):
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_BLOCK_MISMATCH")
    ...
    for key in (
        ...
        if any(dossier[key] != reference[key] for dossier in dossiers):
            raise V8EAuditVerificationBlocked("V8E_AGGREGATE_GATE_BINDING_MISMATCH")
    if any(dossier["window_start"] != aggregate["window_start"] or dossier["window_end_exclusive"] != aggregate["window_end_exclusive"] or dossier["sentinel_indices"] != aggregate["sentinel_indices"] for dossier in dossiers):
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_WINDOW_BINDING_MISMATCH")
    if aggregate["logical_stage"] in READINESS_STAGES:
        ...
        if len(dossiers) != 3 or len(set(coordinates)) != 3 or sorted(coordinates) != list(SENTINEL_INDICES):
            raise V8EAuditVerificationBlocked("V8E_AGGREGATE_SENTINEL_COORDINATE_SET_INVALID")
    ...
    if aggregate["audit_artifact_self_hash"] != artifact_hash:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_ARTIFACT_HASH_MISMATCH")
    ...
    if aggregate["result"] != derived_result:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_RESULT_MISMATCH")
    expected = {
    ...
    for key, value in expected.items():
        if aggregate[key] != value:
            raise V8EAuditVerificationBlocked("V8E_AGGREGATE_DERIVED_VALUE_MISMATCH:" + key)
    if aggregate["result"] == "PASS" and aggregate["sentinel_pass_count"] != aggregate["sentinel_count"]:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_PASS_SENTINEL_MISMATCH")
    if aggregate["audit_evidence_complete"] is not True or aggregate["no_missing_terminal_failure_evidence"] is not True:
        raise V8EAuditVerificationBlocked("V8E_AGGREGATE_EVIDENCE_FLAGS_INVALID")
    return aggregate


# ---------------------------------------------------------------------------
# Reviewed-implementation binding: mechanical derivation for production
# verification (V8E_PROD_HIGH_1B_AUDIT_REVIEW_BINDING_NOT_REDERIVED /
# V8E_PROD_HIGH_1B_REMOVE_PRODUCTION_AUTHORITY_INJECTION_SEAMS)
#
# `derive_reviewed_implementation_commit` -- the PRODUCTION derivation --
# takes no parameters whatsoever: it always resolves the canonical V8E
# repository root and always calls the real HIGH-1A provenance functions
# (`resolve_verified_v8e_production_git_commit`, `verify_frozen_design_
...
def derive_reviewed_implementation_commit() -> str:
    ...
    parameters: always walks verified V8E Git HEAD -> frozen design
    verification -> freeze approval verification -> `src.v8e_production_
    ...
    `V8E_PRODUCTION_IMPLEMENTATION_REVIEW.json` does not exist yet."""
    try:
...
def _derive_reviewed_implementation_commit_via_synthetic_repository_for_tests_only(
    ...
    *,
    ...
    resolver = git_commit_resolver or (lambda: resolve_verified_v8e_production_git_commit(repository_root))
    try:
