"""V8H source-snapshot acquisition support
(`V8H_FRESH_PRIVATE_PARTITION_SUCCESSOR_DESIGN_DRAFT.md` sections 6.1, 7.1).

This module implements ONLY the V8H source-snapshot acquisition stage: the
frozen `HUMAN_V8H_SOURCE_SNAPSHOT_ACQUISITION_GATE` one-shot gate (design
authorization grammar, deterministic fixed receipt key, durable
exclusive/no-overwrite receipt publication), inherited V8 eligible-universe
reconstruction/T0-reproduction reuse (via `src.v8_partition`'s existing
public primitives, unmodified), the fresh-eligible-count ( >= 900 )
fail-closed check, private raw-source-bytes preservation outside Git, and
the safe public source-snapshot evidence artifact producer/verifier. It
does NOT implement V8H partition generation (design section 6.3-6.7,
7.2) -- that is a separate, independently reviewed fresh module per the
design's own two-module split (section 7's "Implementation boundary"),
never merged with this one. It performs no partition allocation, no seed
generation, no membership disclosure, and no research opening.

Bound to the exact independently reviewed and human-frozen design
candidate:

    reviewed_v8h_design_candidate_commit = 543dedf19fe15a94121520e91e441cb751551bc1
    design_blob = c23eb3bdddf320861922e35aa172fa30908a866b
    freeze_approval_blob = 927cf06d84a3e0e80b18305e3b39682fce611b3c

Design section 7.1's receipt-body field list (`source_raw_sha256`,
`source_acquisition_utc`, `eligible_ticker_count`,
`eligible_ticker_list_sha256`, `t0_reproduction_status`) describes values
that can only be known *after* the one authorized JPX request completes
and is parsed -- yet the same section's `consumption_boundary` is
`IMMEDIATELY_BEFORE_FIRST_JPX_REQUEST` and its own text separately
describes "post-gate work" completing only after gate consumption. Every
existing gate module in this repository with an analogous
"immediately before X" boundary (`src/v8f_t1c_preservation.py`'s
`V8F_RECEIPT_FIELDS`, `src/v8g_private_partition_locator.py`'s locator
gate) keeps its durable receipt to pre-known binding values only and
records post-boundary provenance in a *separate* safe evidence artifact
published after the boundary-crossing action succeeds. The current task's
own required-capability list independently confirms this same split by
listing "durable exclusive/no-overwrite gate-receipt publication" and
"canonical public-safe source-snapshot evidence" as two distinct
capabilities. This module therefore implements exactly that split: a
minimal pre-fetch receipt (`consume_gate_once`, published strictly before
the one authorized JPX request, matching the frozen boundary literally),
and a separate post-fetch `V8H_SOURCE_SNAPSHOT_ACQUISITION_EVIDENCE_V1`
artifact carrying the source_raw_sha256/acquisition-UTC/eligible-count/
eligible-list-hash/T0-status fields design section 7.1 lists. No raw
ticker code, private path, or raw payload byte is ever a field in either
artifact.
...
V8H_STUDY_NAME = "V8H_HISTORICAL_RESEARCH"
V8H_REPOSITORY_IDENTITY = "ta1k1-arakawa/stock-analyzer"
...
V8H_AUTHORITATIVE_BRANCH = "v8g-private-partition-locator-successor-design"

# The exact independently reviewed and human-frozen V8H design candidate
# this implementation is bound to (design section 7's "Implementation
# boundary"; freeze recorded in V8H_DESIGN_FREEZE_APPROVAL.json).
REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT = "543dedf19fe15a94121520e91e441cb751551bc1"
V8H_DESIGN_CANDIDATE_BLOB_SHA = "c23eb3bdddf320861922e35aa172fa30908a866b"
V8H_DESIGN_DRAFT_GIT_PATH = "V8H_FRESH_PRIVATE_PARTITION_SUCCESSOR_DESIGN_DRAFT.md"

V8H_FREEZE_APPROVAL_GIT_PATH = "V8H_DESIGN_FREEZE_APPROVAL.json"
V8H_FREEZE_APPROVAL_BLOB_SHA = "927cf06d84a3e0e80b18305e3b39682fce611b3c"

BLOCK_SIZE = 300
MINIMUM_FRESH_ELIGIBLE_COUNT = 900  # 3 x BLOCK_SIZE (design section 6.2)

V8H_SOURCE_SNAPSHOT_GATE = "HUMAN_V8H_SOURCE_SNAPSHOT_ACQUISITION_GATE"
V8H_SOURCE_SNAPSHOT_GATE_CONSUMPTION_BOUNDARY = "IMMEDIATELY_BEFORE_FIRST_JPX_REQUEST"

V8H_AUTHORIZATION_PREFIX = "V8H_HUMAN_AUTHORIZE_SOURCE_SNAPSHOT_ACQUISITION_AT_"
V8H_AUTHORIZATION_WITH = "_WITH_"

V8H_SOURCE_SNAPSHOT_RECEIPT_SCHEMA_VERSION = "V8H_SOURCE_SNAPSHOT_ACQUISITION_GATE_RECEIPT_V1"
V8H_SOURCE_SNAPSHOT_RECEIPT_ARTIFACT_ROLE = "SOURCE_SNAPSHOT_ACQUISITION_GATE_RECEIPT"
V8H_SOURCE_SNAPSHOT_RECEIPT_FIELDS = (
    "schema_version",
    "artifact_role",
    "study",
    "gate",
...
V8H_SOURCE_SNAPSHOT_EVIDENCE_SCHEMA_VERSION = "V8H_SOURCE_SNAPSHOT_ACQUISITION_EVIDENCE_V1"
V8H_SOURCE_SNAPSHOT_EVIDENCE_ARTIFACT_ROLE = "SOURCE_SNAPSHOT_ACQUISITION_EVIDENCE"
V8H_SOURCE_SNAPSHOT_EVIDENCE_FIELDS = (
    "schema_version",
    "artifact_role",
    "study",
    "reviewed_v8h_design_candidate_commit",
...
CANONICAL_V8H_SOURCE_SNAPSHOT_GATE_STATE_ROOT = CANONICAL_CONSUMPTION_STATE_ROOT.parent / "v8h-source-snapshot-gate-state"
CANONICAL_V8H_SOURCE_SNAPSHOT_PRIVATE_STATE_ROOT = (
    CANONICAL_CONSUMPTION_STATE_ROOT.parent / "v8h-source-snapshot-private-state"
)
...
class V8HSourceSnapshotBlocked(RuntimeError):
    """Fail-closed V8H source-snapshot acquisition error."""
...
def _require_hex(value: object, length: int, reason: str) -> str:
    if not isinstance(value, str) or len(value) != length or _HEX.fullmatch(value) is None:
        raise V8HSourceSnapshotBlocked(reason)
    return value
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_NONFINITE_OR_UNSERIALIZABLE") from error
...
def _strict_json_object(raw: bytes, invalid_reason: str, duplicate_reason: str) -> dict[str, Any]:
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise V8HSourceSnapshotBlocked(duplicate_reason)
            result[key] = value
        return result
    ...
    try:
        ...
        raise V8HSourceSnapshotBlocked(invalid_reason) from error
    if not isinstance(parsed, dict):
        raise V8HSourceSnapshotBlocked(invalid_reason)
    return parsed


def _timestamp_text(value: datetime) -> str:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() != timedelta(0):
        raise V8HSourceSnapshotBlocked("V8H_CLOCK_INVALID")
...
def _validate_timestamp(value: object) -> str:
    if not isinstance(value, str) or not (_TIMESTAMP_SECONDS.fullmatch(value) or _TIMESTAMP_MICROS.fullmatch(value)):
        raise V8HSourceSnapshotBlocked("V8H_TIMESTAMP_INVALID")
    try:
        datetime.strptime(value, "%Y-%m-%dT%H:%M:%SZ" if "." not in value else "%Y-%m-%dT%H:%M:%S.%fZ")
    except ValueError as error:
        raise V8HSourceSnapshotBlocked("V8H_TIMESTAMP_INVALID") from error
    return value
...
def _atomic_publish_once(payload: bytes, output: Path, already_exists_reason: str, write_failed_reason: str) -> None:
    ...
    if output.exists():
        raise V8HSourceSnapshotBlocked(already_exists_reason)
    ...
    try:
        ...
        try:
            ...
            raise V8HSourceSnapshotBlocked(already_exists_reason) from error
        except OSError as error:
            raise V8HSourceSnapshotBlocked(write_failed_reason) from error
        _fsync_directory(output.parent)
    except V8HSourceSnapshotBlocked:
        raise
    except OSError as error:
        raise V8HSourceSnapshotBlocked(write_failed_reason) from error
    finally:
...
# ---------------------------------------------------------------------------
# 7.1 -- Human authorization grammar
# ---------------------------------------------------------------------------


def build_authorization_identity(
    *,
    reviewed_v8h_design_candidate_commit: str,
    reviewed_source_snapshot_support_implementation_sha: str,
) -> str:
    candidate = _require_hex(reviewed_v8h_design_candidate_commit, 40, "V8H_DESIGN_CANDIDATE_INVALID")
    implementation = _require_hex(
        reviewed_source_snapshot_support_implementation_sha, 40, "V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_SHA_INVALID"
    )
    return V8H_AUTHORIZATION_PREFIX + candidate + V8H_AUTHORIZATION_WITH + implementation
...
def authorization_identity_sha256(authorization_identity: str) -> str:
    ...
    if not isinstance(authorization_identity, str) or not authorization_identity:
        raise V8HSourceSnapshotBlocked("V8H_AUTHORIZATION_IDENTITY_REQUIRED")
...
def validate_authorization_identity(
    ...
    *,
    reviewed_source_snapshot_support_implementation_sha: str,
    reviewed_v8h_design_candidate_commit: str = REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT,
) -> None:
    """Require the exact V8H source-snapshot authorization grammar and binding."""
    candidate = _require_hex(reviewed_v8h_design_candidate_commit, 40, "V8H_DESIGN_CANDIDATE_INVALID")
    implementation = _require_hex(
        reviewed_source_snapshot_support_implementation_sha, 40, "V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_SHA_INVALID"
    )
    if candidate != REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT:
        raise V8HSourceSnapshotBlocked("V8H_DESIGN_CANDIDATE_MISMATCH")
    if not isinstance(authorization_identity, str) or not authorization_identity:
        raise V8HSourceSnapshotBlocked("V8H_AUTHORIZATION_GRAMMAR_MISMATCH")
    expected = build_authorization_identity(
        reviewed_v8h_design_candidate_commit=candidate,
...
# ---------------------------------------------------------------------------
# 7.1 -- Deterministic one-shot receipt key (independent of authorization,
...
_RECEIPT_KEY_MATERIAL = (
    ...
    + V8H_REPOSITORY_IDENTITY
    + "\0"
    + V8H_STUDY_NAME
    + "\0"
    + V8H_SOURCE_SNAPSHOT_GATE
).encode("utf-8")
...
def compute_source_snapshot_gate_receipt_key() -> str:
    ...
    be durably consumed at most once for the entire life of the V8H study.
    """
...
def _validate_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    if set(receipt) != set(V8H_SOURCE_SNAPSHOT_RECEIPT_FIELDS):
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_SCHEMA_INVALID")
    if receipt["schema_version"] != V8H_SOURCE_SNAPSHOT_RECEIPT_SCHEMA_VERSION:
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_SCHEMA_VERSION_MISMATCH")
    if receipt["artifact_role"] != V8H_SOURCE_SNAPSHOT_RECEIPT_ARTIFACT_ROLE:
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_IDENTITY_INVALID")
    if receipt["study"] != V8H_STUDY_NAME or receipt["gate"] != V8H_SOURCE_SNAPSHOT_GATE:
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_IDENTITY_INVALID")
    _require_hex(receipt["reviewed_v8h_design_candidate_commit"], 40, "V8H_RECEIPT_DESIGN_CANDIDATE_INVALID")
    _require_hex(
        receipt["reviewed_source_snapshot_support_implementation_sha"],
        40,
        "V8H_RECEIPT_IMPLEMENTATION_SHA_INVALID",
    )
    _require_hex(receipt["authorization_identity_sha256"], 64, "V8H_RECEIPT_AUTHORIZATION_HASH_INVALID")
    if (
        receipt["consumed"] is not True
        or type(receipt["consumption_count"]) is not int
        or receipt["consumption_count"] != 1
    ):
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_CONSUMPTION_INVALID")
    if receipt["consumption_boundary"] != V8H_SOURCE_SNAPSHOT_GATE_CONSUMPTION_BOUNDARY:
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_CONSUMPTION_BOUNDARY_INVALID")
...
def read_gate_receipt(state_root: str | os.PathLike[str]) -> dict[str, Any]:
    ...
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_MISSING") from error
    return _validate_receipt(_strict_json_object(raw, "V8H_RECEIPT_INVALID_JSON", "V8H_RECEIPT_DUPLICATE_KEY"))
...
def gate_receipt_bytes_sha256(state_root: str | os.PathLike[str]) -> str:
    ...
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_MISSING") from error
    _validate_receipt(_strict_json_object(raw, "V8H_RECEIPT_INVALID_JSON", "V8H_RECEIPT_DUPLICATE_KEY"))
...
def consume_gate_once(
    ...
    *,
    ...
    """Durably publish exactly one V8H source-snapshot receipt.

    Published strictly before any JPX byte is fetched (design section
    7.1's `consumption_boundary=IMMEDIATELY_BEFORE_FIRST_JPX_REQUEST`), so
    it can contain only pre-known binding values -- never a value that
    depends on the fetch this receipt is about to authorize. No reset or
    replay API exists.
    """
    validate_authorization_identity(
        ...
        reviewed_v8h_design_candidate_commit=reviewed_v8h_design_candidate_commit,
    )
    root = Path(state_root)
    path = _receipt_path(root)
    if path.exists():
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_GATE_ALREADY_CONSUMED")
    receipt = {
        "schema_version": V8H_SOURCE_SNAPSHOT_RECEIPT_SCHEMA_VERSION,
        "artifact_role": V8H_SOURCE_SNAPSHOT_RECEIPT_ARTIFACT_ROLE,
        "study": V8H_STUDY_NAME,
        "gate": V8H_SOURCE_SNAPSHOT_GATE,
        "reviewed_v8h_design_candidate_commit": reviewed_v8h_design_candidate_commit,
        "reviewed_source_snapshot_support_implementation_sha": reviewed_source_snapshot_support_implementation_sha,
        "authorization_identity_sha256": authorization_identity_sha256(authorization_identity),
        "consumed": True,
        "consumption_count": 1,
        "consumption_boundary": V8H_SOURCE_SNAPSHOT_GATE_CONSUMPTION_BOUNDARY,
    ...
    try:
        _atomic_publish_once(
            ...
            path,
            "V8H_SOURCE_SNAPSHOT_GATE_ALREADY_CONSUMED",
            "V8H_RECEIPT_STORAGE_WRITE_FAILED",
        )
    except V8HSourceSnapshotBlocked:
        raise
    return dict(receipt)
...
def _read_and_bind_gate_receipt(
    ...
    *,
    ...
    bound fields and this execution's own authorized values. Returns the
    validated receipt together with the SHA-256 of its exact durable
    bytes -- this is the "point-of-use authorization verification" this
    module provides.
    """
    path = _receipt_path(state_root)
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_MISSING") from error
    receipt = _validate_receipt(_strict_json_object(raw, "V8H_RECEIPT_INVALID_JSON", "V8H_RECEIPT_DUPLICATE_KEY"))
    if receipt["reviewed_v8h_design_candidate_commit"] != reviewed_v8h_design_candidate_commit:
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_DESIGN_CANDIDATE_MISMATCH")
    if (
        receipt["reviewed_source_snapshot_support_implementation_sha"]
        != reviewed_source_snapshot_support_implementation_sha
    ):
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_IMPLEMENTATION_SHA_MISMATCH")
    if receipt["authorization_identity_sha256"] != authorization_identity_sha256(authorization_identity):
        raise V8HSourceSnapshotBlocked("V8H_RECEIPT_AUTHORIZATION_HASH_MISMATCH")
    return receipt, hashlib.sha256(raw).hexdigest()


# ---------------------------------------------------------------------------
# 6.1 -- Inherited V8 eligible-universe reconstruction / T0 reproduction /
# fresh-eligible-count fail-closed check (no allocation, no seed, no tier
# assignment -- those belong to the separate generation module)
# ---------------------------------------------------------------------------
...
def _perform_source_snapshot_acquisition(
    *,
    ...
    if not isinstance(raw_source_bytes, (bytes, bytearray)):
        raise V8HSourceSnapshotBlocked("V8H_RAW_SOURCE_BYTES_INVALID")
    ...
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_" + error.reason) from error
    if sha256_bytes(committed_csv_bytes) != v4_provenance["universe_csv_sha256"]:
        raise V8HSourceSnapshotBlocked("V8H_V4_UNIVERSE_CSV_PROVENANCE_MISMATCH")
    ...
    if not eligible_rows:
        raise V8HSourceSnapshotBlocked("V8H_ELIGIBLE_UNIVERSE_EMPTY")
    ...
    if len(rows_by_code) != len(eligible_rows):
        raise V8HSourceSnapshotBlocked("V8H_ELIGIBLE_LIST_DUPLICATE_TICKER")
    ...
    if len(t0_set) != len(t0_tickers) or len(t0_set) != block_size:
        raise V8HSourceSnapshotBlocked("V8H_T0_SIZE_INVALID")
    ...
    if fresh_eligible_count < minimum_fresh_eligible_count:
        raise V8HSourceSnapshotBlocked("V8H_FRESH_ELIGIBLE_POOL_INSUFFICIENT")
    ...
    if not isinstance(acquired, datetime) or acquired.tzinfo is None or acquired.utcoffset() != timedelta(0):
        raise V8HSourceSnapshotBlocked("V8H_ACQUISITION_TIMESTAMP_INVALID")
...
def preserve_raw_source_bytes_once(
    ...
    """
    digest = _require_hex(source_raw_sha256, 64, "V8H_SOURCE_RAW_SHA_INVALID")
    if not isinstance(raw_source_bytes, (bytes, bytearray)):
        raise V8HSourceSnapshotBlocked("V8H_RAW_SOURCE_BYTES_INVALID")
    ...
    _atomic_publish_once(
        ...
        destination,
        "V8H_PRIVATE_RAW_SOURCE_ALREADY_PRESERVED",
        "V8H_PRIVATE_RAW_SOURCE_WRITE_FAILED",
    )
    return destination


# ---------------------------------------------------------------------------
# 7.1 -- Safe public source-snapshot evidence artifact producer/verifier
# ---------------------------------------------------------------------------
...
def _build_source_snapshot_evidence(
    *,
    ...
    body = {
        "schema_version": V8H_SOURCE_SNAPSHOT_EVIDENCE_SCHEMA_VERSION,
        "artifact_role": V8H_SOURCE_SNAPSHOT_EVIDENCE_ARTIFACT_ROLE,
        "study": V8H_STUDY_NAME,
        "reviewed_v8h_design_candidate_commit": reviewed_v8h_design_candidate_commit,
        "reviewed_source_snapshot_support_implementation_sha": reviewed_source_snapshot_support_implementation_sha,
    ...
    if set(artifact) != set(V8H_SOURCE_SNAPSHOT_EVIDENCE_FIELDS):
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_SCHEMA_INVALID")
    return artifact


def _validate_source_snapshot_evidence(evidence: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(evidence, Mapping) or set(evidence) != set(V8H_SOURCE_SNAPSHOT_EVIDENCE_FIELDS):
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_SCHEMA_INVALID")
    exact = {
        "schema_version": V8H_SOURCE_SNAPSHOT_EVIDENCE_SCHEMA_VERSION,
        "artifact_role": V8H_SOURCE_SNAPSHOT_EVIDENCE_ARTIFACT_ROLE,
    ...
    for key, expected in exact.items():
        if evidence[key] != expected or (isinstance(expected, bool) and type(evidence[key]) is not bool):
            raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_VALUE_INVALID:" + key)
    _require_hex(
        evidence["reviewed_v8h_design_candidate_commit"], 40, "V8H_SOURCE_SNAPSHOT_EVIDENCE_DESIGN_CANDIDATE_INVALID"
    )
    _require_hex(
        evidence["reviewed_source_snapshot_support_implementation_sha"],
        40,
        "V8H_SOURCE_SNAPSHOT_EVIDENCE_IMPLEMENTATION_SHA_INVALID",
    )
    _require_hex(
        evidence["source_snapshot_gate_receipt_key_sha256"],
        64,
        "V8H_SOURCE_SNAPSHOT_EVIDENCE_RECEIPT_KEY_INVALID",
    )
    _require_hex(
        evidence["source_snapshot_gate_receipt_bytes_sha256"],
        64,
        "V8H_SOURCE_SNAPSHOT_EVIDENCE_RECEIPT_BYTES_INVALID",
    )
    _require_hex(evidence["source_raw_sha256"], 64, "V8H_SOURCE_SNAPSHOT_EVIDENCE_SOURCE_RAW_SHA_INVALID")
    _require_hex(
        evidence["eligible_ticker_list_sha256"], 64, "V8H_SOURCE_SNAPSHOT_EVIDENCE_ELIGIBLE_HASH_INVALID"
    )
    _require_hex(evidence["t0_ticker_list_sha256"], 64, "V8H_SOURCE_SNAPSHOT_EVIDENCE_T0_HASH_INVALID")
    ...
    if type(evidence["source_raw_byte_count"]) is not int or evidence["source_raw_byte_count"] < 0:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_BYTE_COUNT_INVALID")
    if type(evidence["eligible_ticker_count"]) is not int or evidence["eligible_ticker_count"] < 1:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_ELIGIBLE_COUNT_INVALID")
    if type(evidence["fresh_eligible_count"]) is not int or evidence["fresh_eligible_count"] < 0:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_FRESH_COUNT_INVALID")
    ...
    if evidence["source_snapshot_artifact_self_sha256"] != recomputed_self_hash:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_SELF_HASH_MISMATCH")
...
def verify_source_snapshot_evidence_binding(
    ...
    *,
    authorized_reviewed_v8h_design_candidate_commit: str,
    authorized_reviewed_source_snapshot_support_implementation_sha: str,
) -> dict[str, Any]:
    """Point-of-use verification for a later stage (e.g. the V8H
    partition-generation gate, design section 7.2) consuming this
    artifact. Independently requires exact equality on the design-
    candidate binding and the source-snapshot-support implementation
    binding, and recomputes the artifact's own self-hash rather than
    trusting the self-declared field alone.
    """
    validated = _validate_source_snapshot_evidence(evidence)
    if validated["reviewed_v8h_design_candidate_commit"] != authorized_reviewed_v8h_design_candidate_commit:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_DESIGN_CANDIDATE_MISMATCH")
    if (
        ...
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_IMPLEMENTATION_MISMATCH")
    if validated["source_snapshot_gate_receipt_key_sha256"] != compute_source_snapshot_gate_receipt_key():
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_RECEIPT_KEY_MISMATCH")
    return validated
...
def _publish_source_snapshot_evidence(evidence: Mapping[str, Any], output: Path) -> dict[str, Any]:
    _atomic_publish_once(
        ...
        output,
        "V8H_SOURCE_SNAPSHOT_EVIDENCE_ALREADY_EXISTS",
        "V8H_SOURCE_SNAPSHOT_EVIDENCE_WRITE_FAILED",
    )
    return dict(evidence)
...
def _validate_public_preflight(preflight: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(preflight, Mapping) or set(preflight) != _PUBLIC_PREFLIGHT_FIELDS:
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_PREFLIGHT_SCHEMA_INVALID")
    if preflight["repository_identity"] != V8H_REPOSITORY_IDENTITY:
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_REPOSITORY_IDENTITY_MISMATCH")
    if preflight["worktree_clean"] is not True:
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_GIT_BINDING_INVALID")
    head = _require_hex(preflight["head"], 40, "V8H_PUBLIC_HEAD_INVALID")
    if preflight["authoritative_remote_head"] != head:
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_HEAD_NOT_AUTHORITATIVE_REMOTE")
    if preflight["reviewed_v8h_design_candidate_commit"] != REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT:
        raise V8HSourceSnapshotBlocked("V8H_DESIGN_CANDIDATE_MISMATCH")
    if preflight["reviewed_v8h_design_blob_sha"] != V8H_DESIGN_CANDIDATE_BLOB_SHA:
        raise V8HSourceSnapshotBlocked("V8H_DESIGN_CANDIDATE_BLOB_MISMATCH")
    if preflight["freeze_approval_blob_sha"] != V8H_FREEZE_APPROVAL_BLOB_SHA:
        raise V8HSourceSnapshotBlocked("V8H_FREEZE_APPROVAL_BLOB_MISMATCH")
    if preflight["freeze_approved_frozen"] is not True:
        raise V8HSourceSnapshotBlocked("V8H_FREEZE_NOT_APPROVED")
...
def _validate_freeze_approval_content(raw: bytes) -> bool:
    ...
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_FREEZE_APPROVAL_INVALID_JSON") from error
    if not isinstance(payload, Mapping):
        raise V8HSourceSnapshotBlocked("V8H_FREEZE_APPROVAL_INVALID_JSON")
    required = (
        payload.get("study") == V8H_STUDY_NAME
        and payload.get("frozen_design_git_commit") == REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT
        and payload.get("frozen_design_git_blob_sha") == V8H_DESIGN_CANDIDATE_BLOB_SHA
...
def _git_text(repository_root: Path, args: list[str], reason: str) -> str:
    try:
        ...
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_GIT_UNAVAILABLE") from error
...
def _default_public_preflight(repository_root: Path = CANONICAL_REPOSITORY_ROOT) -> dict[str, Any]:
    status = _git_text(repository_root, ["status", "--porcelain"], "V8H_PUBLIC_GIT_UNAVAILABLE")
    head = _git_text(repository_root, ["rev-parse", "HEAD"], "V8H_PUBLIC_HEAD_UNAVAILABLE")
    authoritative_remote_head = _git_text(
        repository_root,
        ["rev-parse", "origin/" + V8H_AUTHORITATIVE_BRANCH],
        "V8H_PUBLIC_ORIGIN_UNAVAILABLE",
    )
    origin_url = _git_text(repository_root, ["config", "--get", "remote.origin.url"], "V8H_PUBLIC_ORIGIN_UNAVAILABLE")
    if origin_url not in {
        ...
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_REPOSITORY_IDENTITY_MISMATCH")
    try:
        design_blob = resolve_git_blob(repository_root, REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT, V8H_DESIGN_DRAFT_GIT_PATH)
        freeze_blob = resolve_git_blob(
            repository_root, REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT, V8H_FREEZE_APPROVAL_GIT_PATH
        )
    except V8CGitProvenanceBlocked as error:
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_PROVENANCE_INVALID") from error
    if design_blob != V8H_DESIGN_CANDIDATE_BLOB_SHA:
        raise V8HSourceSnapshotBlocked("V8H_PUBLIC_PROVENANCE_INVALID")
    freeze_approved_frozen = False
    if freeze_blob == V8H_FREEZE_APPROVAL_BLOB_SHA:
        try:
            freeze_raw = read_git_object_bytes(repository_root, REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT, V8H_FREEZE_APPROVAL_GIT_PATH)
        except V8CGitProvenanceBlocked as error:
            raise V8HSourceSnapshotBlocked("V8H_PUBLIC_PROVENANCE_INVALID") from error
        freeze_approved_frozen = _validate_freeze_approval_content(freeze_raw)
    return _validate_public_preflight(
        {
            "repository_identity": V8H_REPOSITORY_IDENTITY,
            "head": head,
            "authoritative_remote_head": authoritative_remote_head,
            "worktree_clean": status == "",
            "reviewed_v8h_design_candidate_commit": REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT,
            "reviewed_v8h_design_blob_sha": design_blob,
...
def _default_reviewed_source_snapshot_support_runtime_state(
    ...
    reviewed_sha = _require_hex(
        reviewed_source_snapshot_support_implementation_sha, 40, "V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_SHA_MALFORMED"
    )
    resolved_sha = _git_text(
        repository_root,
        ["rev-parse", "--verify", f"{reviewed_sha}^{{commit}}"],
        "V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_SHA_UNRESOLVABLE",
    )
    if resolved_sha != reviewed_sha:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_SHA_UNRESOLVABLE")
    count_text = _git_text(
        repository_root,
        ["rev-list", "--count", f"{reviewed_sha}..HEAD"],
        "V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_CHRONOLOGY_INVALID",
    )
    if not count_text.isdecimal():
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_CHRONOLOGY_INVALID")
    return {
        "head": _git_text(repository_root, ["rev-parse", "HEAD"], "V8H_PUBLIC_HEAD_UNAVAILABLE"),
        "authoritative_remote_head": _git_text(
            repository_root,
        ...
        "worktree_clean": _git_text(repository_root, ["status", "--porcelain"], "V8H_PUBLIC_GIT_UNAVAILABLE") == "",
...
def _validate_reviewed_source_snapshot_support_implementation_binding(
    ...
    *,
    ...
    if not isinstance(runtime, Mapping) or set(runtime) != _REVIEWED_IMPLEMENTATION_RUNTIME_FIELDS:
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_RUNTIME_SCHEMA_INVALID")
    if (
        ...
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_IMPLEMENTATION_RUNTIME_BINDING_INVALID")
    return reviewed_sha
...
def _execute_source_snapshot_acquisition_with_dependencies(
    *,
    ...
    the one authorized official-JPX snapshot request.
    """
    ...
    gate_state = _require_safe_external_path(gate_state_root, repository_root, "V8H_GATE_STATE_PATH_INVALID")
    private_state = _require_safe_external_path(private_state_root, repository_root, "V8H_PRIVATE_STATE_PATH_INVALID")
    evidence_output = _require_safe_external_path(evidence_output_path, repository_root, "V8H_OUTPUT_PATH_INVALID")
    if evidence_output.exists():
        raise V8HSourceSnapshotBlocked("V8H_SOURCE_SNAPSHOT_EVIDENCE_ALREADY_EXISTS")
    try:
        gate_state.mkdir(parents=True, exist_ok=True)
        private_state.mkdir(parents=True, exist_ok=True)
        evidence_output.parent.mkdir(parents=True, exist_ok=True)
    except OSError as error:
        raise V8HSourceSnapshotBlocked("V8H_OUTPUT_OR_STATE_PREPARATION_FAILED") from error
...
def resolve_and_acquire_source_snapshot(
    ...
    *,
    ...
    return _execute_source_snapshot_acquisition_with_dependencies(
        authorization_identity=authorization_identity,
        gate_state_root=CANONICAL_V8H_SOURCE_SNAPSHOT_GATE_STATE_ROOT,
        private_state_root=CANONICAL_V8H_SOURCE_SNAPSHOT_PRIVATE_STATE_ROOT,
...
__all__ = [
    "BLOCK_SIZE",
    "CANONICAL_V8H_SOURCE_SNAPSHOT_GATE_STATE_ROOT",
    "CANONICAL_V8H_SOURCE_SNAPSHOT_PRIVATE_STATE_ROOT",
    "MINIMUM_FRESH_ELIGIBLE_COUNT",
    "REVIEWED_V8H_DESIGN_CANDIDATE_COMMIT",
    "V8H_AUTHORITATIVE_BRANCH",
    "V8H_DESIGN_CANDIDATE_BLOB_SHA",
    "V8H_FREEZE_APPROVAL_BLOB_SHA",
    "V8H_SOURCE_SNAPSHOT_EVIDENCE_FIELDS",
    "V8H_SOURCE_SNAPSHOT_GATE",
    "V8H_SOURCE_SNAPSHOT_GATE_CONSUMPTION_BOUNDARY",
    "V8H_SOURCE_SNAPSHOT_RECEIPT_FIELDS",
    "V8H_STUDY_NAME",
    "V8HSourceSnapshotBlocked",
    "authorization_identity_sha256",
