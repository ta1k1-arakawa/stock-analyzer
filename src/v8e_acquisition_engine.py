"""Fixed V8E production raw-acquisition orchestration.

The public functions in :mod:`src.v8e_historical_acquisition` deliberately
...
the existing V8E transport stage remains the sole retry/audit engine.
"""
...
from src.v8e_authority_bridge import T1C_STAGE as T1C_READINESS_STAGE
from src.v8e_authority_bridge import T2_STAGE as T2_READINESS_STAGE
from src.v8e_authority_bridge import verify_stage_authority_bridge
from src.v8e_git_provenance import CANONICAL_REPOSITORY_ROOT, resolve_verified_v8e_production_git_commit
...
from src.v8e_readiness_audit_verification import (
    require_t1c_readiness_audit_verification_pass,
    require_t2_readiness_audit_verification_pass,
)
from src.v8e_t2_point_of_use_preservation import require_t2_point_of_use_preservation_review_pass
...
ACQUISITION_MANIFEST_SCHEMA = "V8E_RAW_ACQUISITION_BUNDLE_V1"
PRODUCTION_BINDING_SCHEMA = "V8E_PRODUCTION_RAW_ACQUISITION_EXECUTION_BINDING_V1"
PRODUCTION_BINDING_ROOT = CANONICAL_CONSUMPTION_STATE_ROOT.parent / "v8e-acquisition-production-execution-state"
...
class V8EAcquisitionEngineBlocked(RuntimeError):
...
def _block(reason: str, error: BaseException | None = None) -> None:
    if error is None:
        raise V8EAcquisitionEngineBlocked(reason)
    raise V8EAcquisitionEngineBlocked(reason) from error
...
def _strict_private_file(path_value: str | os.PathLike[str], label: str) -> Path:
    ...
    if not path.is_absolute() or path.is_symlink():
        _block(f"V8E_ACQUISITION_{label}_LOCATOR_INVALID")
    try:
        ...
        _block(f"V8E_ACQUISITION_{label}_LOCATOR_INVALID", error)
...
def _private_tickers(manifest: Mapping[str, Any], block: str) -> tuple[str, ...]:
    ...
    if not isinstance(assignments, Mapping) or block not in assignments:
        _block("V8E_ACQUISITION_PARTITION_ASSIGNMENT_MISSING")
    ...
    if not isinstance(values, list) or len(values) != REQUEST_COUNT or any(not isinstance(v, str) or not v for v in values):
        _block("V8E_ACQUISITION_PARTITION_COUNT_INVALID")
    if len(set(values)) != len(values):
        _block("V8E_ACQUISITION_PARTITION_DUPLICATE_TICKER")
    ...
    if ticker_list_sha256(values) != expected or manifest.get(field) != expected:
        _block("V8E_ACQUISITION_PARTITION_LIST_HASH_INVALID")
    return tuple(values)
...
def _read_and_validate_partition(path: Path, block: str) -> tuple[dict[str, Any], tuple[str, ...]]:
    ...
    if block == "T1C":
        ...
        if not isinstance(spare, list) or len(spare) != 1904 or ticker_list_sha256(spare) != T1C_PARENT_SPARE_SHA256 or manifest.get("t_spare_ticker_list_sha256") != T1C_PARENT_SPARE_SHA256:
            _block("V8E_ACQUISITION_PARENT_SPARE_HASH_INVALID")
        return manifest, ()
...
def _validate_t1c_allocation(path: Path, manifest: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if artifact.get("t1c_tickers") != expected or artifact.get("remaining_t_spare_tickers") != remaining:
        _block("V8E_ACQUISITION_T1C_MEMBERSHIP_MISMATCH")
    if artifact.get("t1c_ticker_list_sha256") != T1C_LIST_SHA256 or len(expected) != REQUEST_COUNT:
        _block("V8E_ACQUISITION_T1C_LIST_HASH_INVALID")
...
def verify_acquisition_dq_evidence(parsed: Mapping[str, Any], evidence: Mapping[str, Any]) -> None:
    """Recompute canonical DQ evidence before any later opening gate."""
    try:
        validate_dq_evidence(evidence, parsed=parsed)
    except Exception as error:  # noqa: BLE001 - public verification is fail-closed
        _block("V8E_ACQUISITION_DQ_EVIDENCE_MISMATCH", error)


def verify_acquisition_dq_audit_record(parsed: Mapping[str, Any], audit_record: Mapping[str, Any]) -> None:
    """Bind the public audit DQ evidence to independently parsed rows."""
    attempts = audit_record.get("attempts")
    if not isinstance(attempts, list):
        _block("V8E_ACQUISITION_DQ_AUDIT_RECORD_INVALID")
    evidence_values = [
        record.get("detector_evidence")
        for record in attempts
        if isinstance(record, Mapping) and record.get("named_condition") == "DATA_QUALITY_GATE_FAILURE"
    ]
    if len(evidence_values) > 1:
        _block("V8E_ACQUISITION_DQ_AUDIT_EVIDENCE_MULTIPLE")
    expected = derive_dq_failure_evidence(parsed)
    if expected is None:
        if evidence_values:
            _block("V8E_ACQUISITION_DQ_EVIDENCE_MISMATCH")
        return
    if not evidence_values or not isinstance(evidence_values[0], Mapping):
        _block("V8E_ACQUISITION_DQ_EVIDENCE_MISSING")
    verify_acquisition_dq_evidence(parsed, evidence_values[0])
...
def _exclusive_publish_file(destination: Path, payload: bytes) -> None:
    ...
    try:
        ...
        _block("V8E_ACQUISITION_PUBLICATION_PATH_INVALID", error)
    if canonical_destination.parent == canonical_root and canonical_destination.name in PRODUCTION_BINDING_FILENAMES:
        _block("V8E_ACQUISITION_PRODUCTION_BINDING_PUBLICATION_SEAM_BLOCKED")
    ...
    try:
        ...
        try:
            ...
            _block("V8E_ACQUISITION_BUNDLE_ALREADY_EXISTS", error)
        except OSError as error:
            _block("V8E_ACQUISITION_BUNDLE_ATOMIC_PUBLISH_FAILED", error)
    finally:
        try:
            staging.unlink()
...
def _build_bundle(
    *, stage: str, block: str, output_root: Path, staging_root: Path, tickers: tuple[str, ...], captured: Mapping[int, tuple[bytes, Mapping[str, Any]]],
    ...
    if execution["aggregate"]["result"] != "PASS" or len(captured) != REQUEST_COUNT:
        _block("V8E_ACQUISITION_TRANSPORT_BLOCK")
    ...
    _exclusive_publish_file(staging_root / "V8E_ACQUISITION_MANIFEST.json", canonical_json_bytes(manifest))
    output_root.mkdir(parents=True, exist_ok=True)
    final_root = output_root / ("T1C_RAW_ACQUISITION" if block == "T1C" else "T2_RAW_ACQUISITION")
    if final_root.exists() or final_root.is_symlink():
        _block("V8E_ACQUISITION_BUNDLE_ALREADY_EXISTS")
    try:
...
def _execute_fixed_production_acquisition(
    *, stage: str, human_authorization_identity: str, partition_manifest_path: str | os.PathLike[str],
    ...
    if stage not in {"T1C_RAW_ACQUISITION", "T2_RAW_ACQUISITION"}:
        _block("V8E_ACQUISITION_STAGE_INVALID")
    if not isinstance(human_authorization_identity, str) or not human_authorization_identity:
        _block("V8E_ACQUISITION_HUMAN_AUTHORIZATION_IDENTITY_REQUIRED")
    ...
    try:
        ...
        require_gate_not_yet_consumed(CANONICAL_CONSUMPTION_STATE_ROOT, gate_name, EXPECTED_V8E_FROZEN_DESIGN_COMMIT)
        ...
        if anchor.get("authorized_partition_manifest_sha256") != EXPECTED_V8_PARTITION_MANIFEST_SHA256 or anchor.get("authorized_partition_implementation_git_commit") != EXPECTED_V8_PARTITION_IMPLEMENTATION_COMMIT:
            _block("V8E_ACQUISITION_TRUST_ANCHOR_MISMATCH")
        ...
        if stage.startswith("T1C"):
            if t1c_allocation_artifact_path is None:
                _block("V8E_ACQUISITION_T1C_ALLOCATION_REQUIRED")
        ...
        gate_result = consume_gate_and_bind(CANONICAL_CONSUMPTION_STATE_ROOT, logical_stage=stage, v8e_frozen_design_commit=EXPECTED_V8E_FROZEN_DESIGN_COMMIT, reviewed_production_implementation_commit=reviewed_commit, raw_authorization_identity=human_authorization_identity, clock=lambda: datetime.now(timezone.utc))
        ...
        audit_root = CANONICAL_CONSUMPTION_STATE_ROOT.parent / "v8e-acquisition-transport-audit-state" / ("t1c" if stage.startswith("T1C") else "t2")
        execution = execute_v8e_stage(stage=stage, request_factory=request_factory, store=DurableV8EAuditStore(audit_root), reviewed_implementation_commit=reviewed_commit, gate_binding=gate, window_start=REQUEST_START, window_end_exclusive=REQUEST_END_EXCLUSIVE, request_count=REQUEST_COUNT)
        for coordinate, (_raw_bytes, parsed) in captured.items():
            try:
                audit_record = json.loads(Path(execution["dossier_paths"][coordinate]).read_text(encoding="utf-8"))
            except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
                _block("V8E_ACQUISITION_DQ_AUDIT_RECORD_INVALID", error)
            verify_acquisition_dq_audit_record(parsed, audit_record)
        # This binding is deliberately constructed in the fixed production
        # scope, after the durable gate binding and fixed V8E execution exist.
        ...
        if aggregate.get("result") not in {"PASS", "BLOCK"}:
            _block("V8E_ACQUISITION_PRODUCTION_BINDING_RESULT_INVALID")
        ...
        if aggregate_hash != canonical_sha256({key: value for key, value in aggregate.items() if key != "aggregate_self_hash"}):
            _block("V8E_ACQUISITION_PRODUCTION_BINDING_AGGREGATE_HASH_INVALID")
        dossier_bindings = []
        for path_value in execution["dossier_paths"]:
            ...
            if dossier_hash != canonical_sha256({key: value for key, value in dossier.items() if key != "audit_artifact_self_hash"}):
                _block("V8E_ACQUISITION_PRODUCTION_BINDING_DOSSIER_HASH_INVALID")
        ...
        binding_body: dict[str, Any] = {
            ...
            "study": "V8E_HISTORICAL_RESEARCH",
            "artifact_role": "V8E_RAW_ACQUISITION_PRODUCTION_EXECUTION_BINDING",
            "logical_stage": stage,
            "frozen_design_commit": EXPECTED_V8E_FROZEN_DESIGN_COMMIT,
            "reviewed_production_implementation_commit": reviewed_commit,
            "membership_count": REQUEST_COUNT,
        ...
        try:
            ...
            _block("V8E_ACQUISITION_PRODUCTION_BINDING_ALREADY_PUBLISHED", error)
        except OSError as error:
            _block("V8E_ACQUISITION_PRODUCTION_BINDING_ATOMIC_PUBLISH_FAILED", error)
        finally:
        ...
        try:
            ...
            raise
    except V8EAcquisitionEngineBlocked:
        raise
    except Exception as error:  # noqa: BLE001
        _block(getattr(error, "reason", "V8E_ACQUISITION_PRODUCTION_BLOCKED"), error)
    finally:
...
__all__ = [
    "REQUEST_COUNT", "REQUEST_END_EXCLUSIVE", "REQUEST_START", "V8EAcquisitionEngineBlocked",
    "_execute_fixed_production_acquisition", "verify_acquisition_dq_evidence", "verify_acquisition_dq_audit_record",
]
