"""Canonical machine-local bindings for verified V8E readiness audits.
...
from src.v8e_audit import (
    V8EAuditVerificationBlocked,
    verify_aggregate_production,
    verify_dossier_production,
)
from src.v8e_human_gate_consumption import CANONICAL_CONSUMPTION_STATE_ROOT
from src.v8e_readiness import (
...
SCHEMA_VERSION = "V8E_READINESS_AUDIT_VERIFICATION_RECEIPT_V1"
...
CANONICAL_READINESS_AUDIT_VERIFICATION_ROOT = (
    CANONICAL_CONSUMPTION_STATE_ROOT.parent / "v8e-readiness-audit-verification-state"
)
...
def _binding_path_for_stage(logical_stage: str) -> Path:
    ...
    raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_STAGE_INVALID")


class V8EReadinessAuditVerificationBlocked(RuntimeError):
...
def _strict_json_object(raw: bytes, *, invalid_reason: str, duplicate_reason: str) -> dict[str, Any]:
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, item in pairs:
            if key in value:
                raise V8EReadinessAuditVerificationBlocked(duplicate_reason)
            value[key] = item
        return value
    ...
    try:
        ...
        raise V8EReadinessAuditVerificationBlocked(invalid_reason) from error
    if not isinstance(value, dict):
        raise V8EReadinessAuditVerificationBlocked(invalid_reason)
    return value
...
def _require_hex(value: object, length: int, reason: str) -> str:
    if not isinstance(value, str) or len(value) != length or any(char not in "0123456789abcdef" for char in value):
        raise V8EReadinessAuditVerificationBlocked(reason)
    return value
...
def _stage_constants(logical_stage: str) -> tuple[str, str, Path]:
    ...
    raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_VERIFICATION_STAGE_INVALID")
...
def _canonical_root(root: Path) -> Path:
    try:
        ...
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_CANONICAL_ROOT_UNAVAILABLE") from error
...
def _safe_audit_file(path_value: str | Path, audit_root: Path, *, reason: str) -> tuple[Path, str]:
    try:
        ...
        raise V8EReadinessAuditVerificationBlocked(reason) from error
...
def _read_json_file(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise V8EReadinessAuditVerificationBlocked(f"V8E_READINESS_AUDIT_{label}_READ_FAILED") from error
    return _strict_json_object(
        raw,
        invalid_reason=f"V8E_READINESS_AUDIT_{label}_INVALID_JSON",
        duplicate_reason=f"V8E_READINESS_AUDIT_{label}_DUPLICATE_KEY",
    )
...
def _read_production_execution_binding(logical_stage: str) -> dict[str, Any]:
    ...
    except (OSError, V8EReadinessAuditVerificationBlocked) as error:
        if isinstance(error, V8EReadinessAuditVerificationBlocked):
            raise
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_READ_FAILED") from error
    if set(binding) != set(PRODUCTION_EXECUTION_BINDING_FIELDS):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_SCHEMA_INVALID")
    if (
        ...
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_PROVENANCE_INVALID")
    _require_hex(
        binding["reviewed_production_implementation_commit"], 40,
        "V8E_READINESS_PRODUCTION_BINDING_IMPLEMENTATION_INVALID",
    )
    _require_hex(binding["aggregate_artifact_self_hash"], 64, "V8E_READINESS_PRODUCTION_BINDING_AGGREGATE_HASH_INVALID")
    for key in ("gate_receipt_key_sha256", "gate_receipt_bytes_sha256", "authorization_identity_sha256"):
        _require_hex(binding[key], 64, "V8E_READINESS_PRODUCTION_BINDING_GATE_INVALID")
    if (
        ...
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_FILENAME_INVALID")
    dossier_bindings = binding["dossier_bindings"]
    if not isinstance(dossier_bindings, list) or len(dossier_bindings) != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_DOSSIER_COUNT_INVALID")
    coordinates: list[int] = []
    for dossier in dossier_bindings:
        if not isinstance(dossier, dict) or set(dossier) != set(PRODUCTION_EXECUTION_DOSSIER_FIELDS):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_DOSSIER_SCHEMA_INVALID")
        if (
            ...
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_DOSSIER_INVALID")
        _require_hex(dossier["audit_artifact_self_hash"], 64, "V8E_READINESS_PRODUCTION_BINDING_DOSSIER_HASH_INVALID")
    ...
    if coordinates != list(SENTINEL_INDICES) or len({item["filename"] for item in dossier_bindings}) != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_COORDINATES_INVALID")
    if binding["binding_self_hash"] != canonical_sha256(
        {key: value for key, value in binding.items() if key != "binding_self_hash"}
    ):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_SELF_HASH_INVALID")
    return binding
...
def _require_matching_production_execution_binding(
    *, binding: Mapping[str, Any], logical_stage: str, aggregate: Mapping[str, Any],
    ...
    if binding["logical_stage"] != logical_stage or binding["aggregate_filename"] != aggregate_filename:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_STAGE_MISMATCH")
    for key in (
        ...
        if binding[key] != aggregate.get(key):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_AGGREGATE_MISMATCH")
    ...
    if binding["aggregate_artifact_self_hash"] != aggregate.get("aggregate_self_hash"):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_AGGREGATE_HASH_MISMATCH")
    ...
    for declared, dossier, filename, coordinate in zip(
        ...
        if (
            ...
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_DOSSIER_MISMATCH")
    ...
    for key in ("gate_receipt_key_sha256", "gate_receipt_bytes_sha256", "authorization_identity_sha256"):
        if binding[key] != gate_binding.get(key):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_PRODUCTION_BINDING_GATE_MISMATCH")
...
def _require_verified_dossier_bindings(
    ...
    if len(dossiers) != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DOSSIER_COUNT_INVALID")
    bindings: list[dict[str, str]] = []
    reference: dict[str, str] | None = None
    for dossier in dossiers:
        if dossier.get("logical_stage") != logical_stage:
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DOSSIER_STAGE_INVALID")
        binding = {
            "gate_receipt_key_sha256": _require_hex(
                dossier.get("gate_receipt_key_sha256"), 64, "V8E_READINESS_AUDIT_GATE_KEY_INVALID"
            ),
            "gate_receipt_bytes_sha256": _require_hex(
                dossier.get("gate_receipt_bytes_sha256"), 64, "V8E_READINESS_AUDIT_GATE_BYTES_HASH_INVALID"
            ),
            "authorization_identity_sha256": _require_hex(
                dossier.get("authorization_identity_sha256"), 64, "V8E_READINESS_AUDIT_AUTHORIZATION_HASH_INVALID"
            ),
            "audit_artifact_self_hash": _require_hex(
                dossier.get("audit_artifact_self_hash"), 64, "V8E_READINESS_AUDIT_DOSSIER_HASH_INVALID"
            ),
        }
        if reference is None:
            ...
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_GATE_BINDING_MISMATCH")
    ...
    if aggregate.get("result") != VERIFICATION_RESULT:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RESULT_NOT_PASS")
    if reference is None:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DOSSIER_COUNT_INVALID")
...
def _require_readiness_pass(aggregate: Mapping[str, Any], logical_stage: str) -> None:
    if aggregate.get("logical_stage") != logical_stage:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_STAGE_MISMATCH")
    if aggregate.get("result") != VERIFICATION_RESULT:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RESULT_NOT_PASS")
    if aggregate.get("sentinel_indices") != list(SENTINEL_INDICES):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_SENTINEL_BINDING_INVALID")
    if aggregate.get("sentinel_count") != 3 or aggregate.get("sentinel_pass_count") != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_SENTINEL_PASS_COUNT_INVALID")
    if aggregate.get("window_start") != SENTINEL_START or aggregate.get("window_end_exclusive") != SENTINEL_END_EXCLUSIVE:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_WINDOW_BINDING_INVALID")
    if aggregate.get("frozen_design_commit") != FROZEN_DESIGN_COMMIT:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DESIGN_BINDING_INVALID")
    if aggregate.get("canonical_parser_classifier_commit") != CANONICAL_PARSER_CLASSIFIER_COMMIT:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_CLASSIFIER_COMMIT_INVALID")
    if aggregate.get("canonical_parser_classifier_blob") != CANONICAL_PARSER_CLASSIFIER_BLOB:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_CLASSIFIER_BLOB_INVALID")
...
def _build_receipt(
    *,
    ...
    if len(dossier_bindings) != 3 or any(set(binding) != set(DOSSIER_BINDING_FIELDS) for binding in dossier_bindings):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DOSSIER_BINDINGS_INVALID")
    body: dict[str, Any] = {
        ...
        "reviewed_production_implementation_commit": _require_hex(
            ...
            "V8E_READINESS_AUDIT_IMPLEMENTATION_SHA_INVALID",
        ),
        "aggregate_filename": aggregate_filename,
        "aggregate_artifact_self_hash": _require_hex(
            aggregate.get("aggregate_self_hash"), 64, "V8E_READINESS_AUDIT_AGGREGATE_HASH_INVALID"
        ),
        "dossier_bindings": [dict(binding) for binding in dossier_bindings],
        "gate_receipt_key_sha256": _require_hex(
            gate_binding.get("gate_receipt_key_sha256"), 64, "V8E_READINESS_AUDIT_GATE_KEY_INVALID"
        ),
        "gate_receipt_bytes_sha256": _require_hex(
            gate_binding.get("gate_receipt_bytes_sha256"), 64, "V8E_READINESS_AUDIT_GATE_BYTES_HASH_INVALID"
        ),
        "authorization_identity_sha256": _require_hex(
            gate_binding.get("authorization_identity_sha256"), 64,
            "V8E_READINESS_AUDIT_AUTHORIZATION_HASH_INVALID",
        ),
    }
...
def _validate_receipt(receipt: Mapping[str, Any], *, expected_verification_stage: str) -> dict[str, Any]:
    if set(receipt) != set(RECEIPT_FIELDS):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_SCHEMA_INVALID")
    ...
    if receipt["schema_version"] != SCHEMA_VERSION or receipt["study"] != STUDY:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_PROVENANCE_INVALID")
    if receipt["artifact_role"] != ARTIFACT_ROLE or receipt["verification_result"] != VERIFICATION_RESULT:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_RESULT_INVALID")
    if receipt["verification_stage"] != verification_stage or receipt["logical_stage"] != logical_stage:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_STAGE_MISMATCH")
    if receipt["frozen_design_commit"] != FROZEN_DESIGN_COMMIT:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_DESIGN_MISMATCH")
    _require_hex(receipt["reviewed_production_implementation_commit"], 40, "V8E_READINESS_AUDIT_RECEIPT_IMPLEMENTATION_INVALID")
    _require_hex(receipt["aggregate_artifact_self_hash"], 64, "V8E_READINESS_AUDIT_RECEIPT_AGGREGATE_HASH_INVALID")
    _require_hex(receipt["gate_receipt_key_sha256"], 64, "V8E_READINESS_AUDIT_RECEIPT_GATE_KEY_INVALID")
    _require_hex(receipt["gate_receipt_bytes_sha256"], 64, "V8E_READINESS_AUDIT_RECEIPT_GATE_BYTES_INVALID")
    _require_hex(receipt["authorization_identity_sha256"], 64, "V8E_READINESS_AUDIT_RECEIPT_AUTHORIZATION_INVALID")
    if receipt["receipt_self_hash"] != canonical_sha256({key: value for key, value in receipt.items() if key != "receipt_self_hash"}):
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_SELF_HASH_INVALID")
    dossier_bindings = receipt["dossier_bindings"]
    if not isinstance(dossier_bindings, list) or len(dossier_bindings) != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_DOSSIER_BINDINGS_INVALID")
    filenames: list[str] = []
    for binding in dossier_bindings:
        if not isinstance(binding, dict) or set(binding) != set(DOSSIER_BINDING_FIELDS):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_DOSSIER_BINDING_SCHEMA_INVALID")
        if (
            not isinstance(binding["filename"], str)
            or not binding["filename"]
            or Path(binding["filename"]).name != binding["filename"]
        ):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_FILENAME_INVALID")
        filenames.append(binding["filename"])
        _require_hex(binding["audit_artifact_self_hash"], 64, "V8E_READINESS_AUDIT_RECEIPT_DOSSIER_HASH_INVALID")
    if len(set(filenames)) != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_DOSSIER_DUPLICATE")
    if (
...
def _persist_receipt(receipt: dict[str, Any], destination: Path) -> dict[str, Any]:
    def existing_or_conflict() -> dict[str, Any]:
        if destination.is_symlink():
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_PATH_INVALID")
        ...
        if existing != receipt:
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_CONFLICT")
        return existing
    ...
    try:
        ...
        try:
            ...
            if temporary.exists():
                temporary.unlink()
    except V8EReadinessAuditVerificationBlocked:
        raise
    except (OSError, TypeError, ValueError) as error:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_RECEIPT_WRITE_FAILED") from error
...
def _write_verified_receipt(
    *,
    ...
    aggregate_file, aggregate_filename = _safe_audit_file(
        aggregate_path, audit_root, reason="V8E_READINESS_AUDIT_AGGREGATE_PATH_INVALID"
    )
    ...
    for path in dossier_paths:
        file_path, filename = _safe_audit_file(
            path, audit_root, reason="V8E_READINESS_AUDIT_DOSSIER_PATH_INVALID"
        )
        dossier_files.append(file_path)
        dossier_filenames.append(filename)
    if len(set(dossier_filenames)) != 3:
        raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DOSSIER_DUPLICATE")
    try:
    ...
    except (V8EAuditVerificationBlocked, V8EReadinessAuditVerificationBlocked) as error:
        if isinstance(error, V8EReadinessAuditVerificationBlocked):
            raise
        raise V8EReadinessAuditVerificationBlocked(error.reason) from error
...
def _read_canonical_pass(verification_stage: str) -> dict[str, Any]:
    ...
    try:
        ...
        aggregate_path, _ = _safe_audit_file(
            ...
            reason="V8E_READINESS_AUDIT_AGGREGATE_PATH_INVALID",
        )
        ...
        for binding in receipt["dossier_bindings"]:
            dossier_path, _ = _safe_audit_file(
                ...
                _canonical_root(CANONICAL_PRODUCTION_AUDIT_ROOT),
                reason="V8E_READINESS_AUDIT_DOSSIER_PATH_INVALID",
            )
        ...
        for actual, declared in zip(actual_bindings, receipt["dossier_bindings"]):
            if actual["audit_artifact_self_hash"] != declared["audit_artifact_self_hash"]:
                raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_DOSSIER_BINDING_MISMATCH")
        if receipt["aggregate_artifact_self_hash"] != aggregate.get("aggregate_self_hash"):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_AGGREGATE_BINDING_MISMATCH")
        if receipt["reviewed_production_implementation_commit"] != aggregate.get("reviewed_production_implementation_commit"):
            raise V8EReadinessAuditVerificationBlocked("V8E_READINESS_AUDIT_IMPLEMENTATION_BINDING_MISMATCH")
        for key in ("gate_receipt_key_sha256", "gate_receipt_bytes_sha256", "authorization_identity_sha256"):
        ...
        raise
    except (V8EAuditVerificationBlocked, OSError, TypeError, ValueError) as error:
        raise V8EReadinessAuditVerificationBlocked(
            getattr(error, "reason", "V8E_READINESS_AUDIT_VERIFICATION_BLOCKED")
        ) from error
