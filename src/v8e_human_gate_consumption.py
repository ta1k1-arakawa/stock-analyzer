"""Durable, fail-closed, one-shot consumption receipts for V8E's four
Yahoo-request-bearing production human gates.

`V8E_DQ_EVIDENCE_SUCCESSOR_DESIGN_DRAFT.md`. Mirrors
...
with V8E-specific constants, an exact 12-field receipt schema, and simpler
one-shot semantics: **every** V8E gate below is one-shot for the life of the
frozen V8E design commit -- unlike V8C's readiness gates, there is no
per-authorization-identity exception. A fresh authorization identity must
never reset an already-consumed V8E stage; the deterministic receipt key is
therefore derived from ``(repository, gate, v8e_frozen_design_commit)``
...
STUDY_NAME = "V8E_HISTORICAL_RESEARCH"
SCHEMA_VERSION = "V8E_HUMAN_GATE_CONSUMPTION_RECEIPT_V1"
...
def _resolve_windows_program_data_directory() -> Path:
    try:
        ...
        raise RuntimeError("V8E_HUMAN_GATE_STATE_ROOT_UNAVAILABLE") from error


def _default_consumption_state_root() -> Path:
    if os.name == "nt":
        base = _resolve_windows_program_data_directory()
        return base / "stock-analyzer" / "v8e-human-gate-state"
    return _POSIX_MACHINE_STATE_BASE / "v8e-human-gate-state"


CANONICAL_CONSUMPTION_STATE_ROOT = _default_consumption_state_root()


class V8EHumanGateConsumptionBlocked(RuntimeError):
    """Fail-closed durable one-shot V8E human-gate consumption error."""
...
def _require_known_stage(value: object) -> str:
    if value not in KNOWN_STAGES:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_STAGE_UNKNOWN")
    return value


def _require_known_gate(value: object) -> str:
    if value not in KNOWN_GATES:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_UNKNOWN")
    return value
...
def _require_stage_gate_pair(stage: object, gate: object) -> tuple[str, str]:
    ...
    if STAGE_GATE[stage] != gate:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_STAGE_GATE_MISMATCH")
    return stage, gate
...
def _require_git_commit(value: object, reason: str) -> str:
    if not isinstance(value, str) or len(value) != 40 or any(char not in "0123456789abcdef" for char in value):
        raise V8EHumanGateConsumptionBlocked(reason)
    return value
...
def compute_receipt_key(gate: str, v8e_frozen_design_commit: str) -> str:
    """Deterministic receipt key from only safe components: repository,
    gate, and the exact frozen V8E design commit -- deliberately excluding
    ...
    _require_git_commit(v8e_frozen_design_commit, "V8E_HUMAN_GATE_DESIGN_COMMIT_INVALID")
    material = REPOSITORY_IDENTITY + "|" + gate + "|" + v8e_frozen_design_commit
    return hashlib.sha256(material.encode("utf-8")).hexdigest()


def _receipt_path(state_root: str | os.PathLike[str], gate: str, v8e_frozen_design_commit: str) -> Path:
    return Path(state_root) / (compute_receipt_key(gate, v8e_frozen_design_commit) + ".json")
...
def has_gate_been_consumed(state_root: str | os.PathLike[str], gate: str, v8e_frozen_design_commit: str) -> bool:
    """Read-only existence check -- never raises for "not yet consumed";
    only raises for a malformed ``gate``/``v8e_frozen_design_commit`` or an
    unreadable state root."""
    path = _receipt_path(state_root, gate, v8e_frozen_design_commit)
    try:
        return path.exists()
    except OSError as error:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_STATE_UNAVAILABLE") from error


def require_gate_not_yet_consumed(state_root: str | os.PathLike[str], gate: str, v8e_frozen_design_commit: str) -> None:
    """Fail-fast, read-only pre-check: BLOCK immediately if ``gate`` (under
    this exact key) has already been durably consumed."""
    if has_gate_been_consumed(state_root, gate, v8e_frozen_design_commit):
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_ALREADY_CONSUMED:" + gate)
...
def _reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_DUPLICATE_KEY")
        result[key] = value
    return result
...
def _parse_and_validate_receipt_bytes(raw: bytes) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_INVALID_JSON") from error
    if not isinstance(parsed, dict) or set(parsed) != set(RECEIPT_FIELDS):
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_SCHEMA_INVALID")
    if parsed["schema_version"] != SCHEMA_VERSION:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_SCHEMA_VERSION_MISMATCH")
    if parsed["study"] != STUDY_NAME:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_STUDY_MISMATCH")
    if parsed["repository"] != REPOSITORY_IDENTITY:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_REPOSITORY_MISMATCH")
    _require_stage_gate_pair(parsed["logical_stage"], parsed["gate"])
    _require_git_commit(parsed["v8e_frozen_design_commit"], "V8E_HUMAN_GATE_RECEIPT_DESIGN_COMMIT_INVALID")
    _require_git_commit(
        parsed["reviewed_production_implementation_commit"], "V8E_HUMAN_GATE_RECEIPT_IMPLEMENTATION_COMMIT_INVALID"
    )
    _require_hex64(parsed["authorization_identity_sha256"], "V8E_HUMAN_GATE_RECEIPT_AUTHORIZATION_HASH_INVALID")
    if parsed["consumed"] is not True:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_CONSUMED_FLAG_INVALID")
    if type(parsed["consumption_count"]) is not int or parsed["consumption_count"] != 1:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_CONSUMPTION_COUNT_INVALID")
    if parsed["consumption_boundary"] != CONSUMPTION_BOUNDARY:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_CONSUMPTION_BOUNDARY_INVALID")
    _require_canonical_utc_timestamp(parsed["consumed_at_utc"])

    recomputed_key = compute_receipt_key(parsed["gate"], parsed["v8e_frozen_design_commit"])
...
def _read_and_validate_receipt(state_root: str | os.PathLike[str], receipt_key: str) -> tuple[dict[str, Any], bytes]:
    if not isinstance(receipt_key, str) or len(receipt_key) != 64 or any(
        ...
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_KEY_INVALID")
    path = Path(state_root) / (receipt_key + ".json")
    try:
        raw = path.read_bytes()
    except OSError as error:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_MISSING") from error
    ...
    if recomputed_key != receipt_key:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_KEY_CONTENT_MISMATCH")
    return parsed, raw
...
def read_gate_consumption_receipt(
    ...
    *,
    ...
    expected_v8e_frozen_design_commit: str | None = None,
) -> dict[str, Any]:
    ...
    if expected_gate is not None and parsed["gate"] != expected_gate:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_GATE_MISMATCH")
    if expected_v8e_frozen_design_commit is not None and parsed["v8e_frozen_design_commit"] != expected_v8e_frozen_design_commit:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_DESIGN_COMMIT_MISMATCH")
    return parsed
...
def _utc_timestamp(value: Any) -> datetime:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() != timedelta(0):
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_CLOCK_INVALID")
...
def _require_canonical_utc_timestamp(value: object) -> str:
    if not isinstance(value, str) or not _CANONICAL_UTC_TIMESTAMP.fullmatch(value):
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_TIMESTAMP_INVALID")
    try:
        ...
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_TIMESTAMP_INVALID") from error
...
def consume_gate_and_bind(
    ...
    *,
    logical_stage: str,
    v8e_frozen_design_commit: str,
    reviewed_production_implementation_commit: str,
    raw_authorization_identity: str,
    clock: Callable[[], datetime],
) -> V8EGateReceiptBinding:
    """The sole function that durably, atomically, fail-closed consumes a
    V8E human gate exactly once. The caller must invoke this exactly once,
    ...
    _require_git_commit(v8e_frozen_design_commit, "V8E_HUMAN_GATE_DESIGN_COMMIT_INVALID")
    _require_git_commit(
        reviewed_production_implementation_commit, "V8E_HUMAN_GATE_IMPLEMENTATION_COMMIT_INVALID"
    )
    if not isinstance(raw_authorization_identity, str) or not raw_authorization_identity:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_AUTHORIZATION_IDENTITY_REQUIRED")
    ...
    receipt = {
        ...
        "gate": gate,
        "logical_stage": stage,
        "v8e_frozen_design_commit": v8e_frozen_design_commit,
    ...
    try:
        try:
            ...
            raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_STATE_WRITE_FAILED") from error
        try:
            os.link(str(staging), str(path))
        except FileExistsError as error:
            raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_ALREADY_CONSUMED:" + gate) from error
        except OSError as error:
            raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_STATE_WRITE_FAILED") from error
    finally:
        try:
    ...
    receipt_key = compute_receipt_key(gate, v8e_frozen_design_commit)
    validated, bytes_hash = read_gate_consumption_receipt_with_bytes_hash(
        state_root, receipt_key, expected_gate=gate, expected_v8e_frozen_design_commit=v8e_frozen_design_commit,
    )
    if validated["logical_stage"] != stage:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_STAGE_MISMATCH")
    if validated["reviewed_production_implementation_commit"] != reviewed_production_implementation_commit:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_IMPLEMENTATION_MISMATCH")
    if validated["authorization_identity_sha256"] != identity_hash:
        raise V8EHumanGateConsumptionBlocked("V8E_HUMAN_GATE_RECEIPT_AUTHORIZATION_MISMATCH")

    return V8EGateReceiptBinding(
        human_gate=gate,
        logical_stage=stage,
