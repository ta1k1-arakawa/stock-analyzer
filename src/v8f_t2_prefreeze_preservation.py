...
The exact V8F T2 record is produced in memory for synthetic verification
only; the committed checkpoint is a later separately authorized stage.

Remediation note (V8F-PREFREEZE-HIGH-001): the canonical production path
(``resolve_and_verify_t2_prefreeze_preservation`` /
``_resolve_t2_prefreeze_safe_evidence_with_dependencies``) independently
derives the nine preservation conditions and required V8 provenance from the
applicable committed public Git objects, mechanically rebinding the V8E
precedent's derivation architecture (``src/v8e_t2_prefreeze_preservation.py``)
into the V8F namespace.  A caller-supplied final safe-evidence mapping is
never sufficient on its own to reach the canonical PASS path; it is accepted
only by the pure, already-derived-evidence validators
(``_validate_safe_evidence`` / ``verify_t2_prefreeze_record`` /
``build_t2_prefreeze_record``) for synthetic unit testing, exactly as the
V8E precedent's own test suite already exercises that layer.

The historical predecessor documents bound here are the real, currently
committed V8E artifacts supplied for this task: the V8E T1C readiness
terminal adjudication (proving the whole V8E study never advanced past T1C
readiness, and therefore never reached T2) and V8E's own T2 prefreeze
...
V8F_DESIGN_GIT_PATH = "V8F_TRANSPORT_WINDOW_SEMANTICS_SUCCESSOR_DESIGN_DRAFT.md"
...
# Real, currently committed public evidence sources.  Blobs are exact and
# fixed; a source whose current blob differs from these is BLOCK, not a
# silent re-derivation from whatever is currently on disk.
...
# Historical V8E predecessor-study terminal disposition.  Fixed historical
# commit/blob supplied for this task; proves the whole V8E study terminated
# at T1C readiness BLOCK and therefore never reached T2.
...
# Historical V8E T2 prefreeze preservation record: the direct predecessor-
# study analogue of the V8D record the V8E precedent itself bound to.  Fixed
# to its real Git introduction commit (`git log --follow`), not the moving
# V8F head, since it is genuinely historical evidence.
...
# ---------------------------------------------------------------------------
# Pure validators.  These accept an already-derived safe-evidence mapping and
# are used both by the canonical path (fed with Git-derived evidence below)
# and directly by synthetic unit tests.  They never themselves establish
# canonical PASS from a bare caller assertion outside a test context.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Committed public Git evidence readers and validators.  These derive
# observed facts from real repository objects; nothing here accepts a
# caller-asserted boolean/hash as sufficient evidence on its own.
# ---------------------------------------------------------------------------
...
def _parse_first_text_block(raw: bytes, reason: str) -> dict[str, str]:
    try:
        ...
        raise V8FT2PrefreezePreservationBlocked(reason) from error
...
def _validate_v8b_bridge(bridge: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if set(bridge) != required:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_BRIDGE_SCHEMA_INVALID")
    expected = {
    ...
    for key, value in expected.items():
        if type(bridge[key]) is not type(value) or bridge[key] != value:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_BRIDGE_VALUE_MISMATCH:" + key)
    if not isinstance(bridge["authorization_note"], str) or "never mutates" not in bridge["authorization_note"]:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_BRIDGE_NOTE_INVALID")
    return dict(bridge)
...
def _validate_anchor(anchor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if set(anchor) != required:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_ANCHOR_SCHEMA_INVALID")
    exact = {
    ...
    for key, value in exact.items():
        if anchor[key] != value:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_ANCHOR_VALUE_MISMATCH:" + key)
    if not isinstance(anchor["authorization_note"], str) or "does NOT authorize T2 acquisition" not in anchor["authorization_note"]:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_ANCHOR_NOTE_INVALID")
    return dict(anchor)


def _validate_v8e_terminal(raw: bytes) -> dict[str, Any]:
    """Validate the historical V8E T1C readiness terminal adjudication.

    Proves the whole V8E study terminated at T1C readiness BLOCK, so T2 was
    structurally never reached under the frozen V8E/V8F stage order.
    """
    parsed = _strict_json_object(raw, "V8F_T2_V8E_TERMINAL_INVALID_JSON", "V8F_T2_V8E_TERMINAL_DUPLICATE_KEY")
    exact = {
    ...
    for key, expected in exact.items():
        if parsed.get(key) != expected:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_V8E_TERMINAL_MISMATCH")
    return {"t2_features_observed": False, "t2_outcomes_observed": False, "t2_access_prohibited": True}


_V8E_T2_HISTORICAL_FIELDS = frozenset(
    field if field != "reviewed_v8f_design_candidate_commit" else "reviewed_v8e_design_candidate_commit"
    for field in V8F_T2_PREFREEZE_RECORD_FIELDS
)


def _validate_historical_v8e_t2_record(raw: bytes) -> dict[str, Any]:
    values = _parse_first_text_block(raw, "V8F_T2_V8E_HISTORICAL_RECORD_INVALID")
    if set(values) != _V8E_T2_HISTORICAL_FIELDS:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_V8E_HISTORICAL_SCHEMA_INVALID")
    expected = {
        "study": "V8E_HISTORICAL_RESEARCH",
        "document_type": V8F_T2_PREFREEZE_DOCUMENT_TYPE,
    ...
    if values != expected:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_V8E_HISTORICAL_VALUE_MISMATCH")
    return {
        **values,
        "T2_real_data_acquired": False,
        "T2_opened": False,
...
def _validate_v8f_design(raw: bytes) -> dict[str, bool]:
    required = (
    ...
    try:
        ...
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_DESIGN_INVALID") from error
    if any(value not in text for value in required):
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_DESIGN_POLICY_INVALID")
...
def _derive_safe_evidence(
    ...
    *,
    ...
    state = _require_mapping(state, "V8F_T2_STATE_EVIDENCE_INVALID")
    bridge = _validate_v8b_bridge(_require_mapping(bridge, "V8F_T2_BRIDGE_EVIDENCE_INVALID"))
    anchor = _validate_anchor(_require_mapping(anchor, "V8F_T2_ANCHOR_EVIDENCE_INVALID"))
    if not all(isinstance(value, Mapping) for value in (historical_t2, terminal, design)):
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_REQUIRED_SAFE_EVIDENCE_MISSING")
    historical_t2 = dict(historical_t2)
    terminal = dict(terminal)
    if terminal.get("t2_access_prohibited") is not True or design.get("policy_unchanged") is not True:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_REQUIRED_SAFE_EVIDENCE_INVALID")
    t2 = _require_mapping(state.get("T2"), "V8F_T2_STATE_SECTION_INVALID")
    trust = _require_mapping(state.get("trust_anchor_pinning"), "V8F_T2_STATE_TRUST_SECTION_INVALID")
    partition = _require_mapping(state.get("partition"), "V8F_T2_STATE_PARTITION_SECTION_INVALID")
    history_value = state.get("real_partition_build_history")
    if not isinstance(history_value, list):
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_STATE_HISTORY_NOT_LIST")
    if len(history_value) != 1:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_STATE_HISTORY_ENTRY_COUNT_INVALID")
    history = _require_mapping(history_value[0], "V8F_T2_STATE_HISTORY_ENTRY_INVALID")
    history_expected = {
    ...
    for key, value in history_expected.items():
        if type(history.get(key)) is not type(value) or history.get(key) != value:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_HISTORY_VALUE_MISMATCH:" + key)
    ...
    if not isinstance(block_sizes, Mapping) or dict(block_sizes) != {"T0": 300, "T1": 300, "T2": 300, "T3": 300, "T_spare": 1904}:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_HISTORY_BLOCK_SIZES_INVALID")
    state_exact = {
    ...
    for key, value in state_exact.items():
        if type(state.get(key)) is not type(value) or state.get(key) != value:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_STATE_VALUE_MISMATCH:" + key)
    t2_expected = {
    ...
    for key, value in t2_expected.items():
        if type(t2.get(key)) is not type(value) or t2.get(key) != value:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_STATE_T2_VALUE_MISMATCH:" + key)
    if trust.get("block_assignments_exposed") is not False:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_ASSIGNMENTS_EXPOSED")
    partition_expected = {
        "manifest_sha256": EXPECTED_V8_PARTITION_MANIFEST_SHA256,
        "partition_implementation_git_commit": EXPECTED_V8_PARTITION_IMPLEMENTATION_COMMIT,
        "manifest_schema_version": "V8_PARTITION_MANIFEST_V3",
    ...
    for key, value in partition_expected.items():
        if type(partition.get(key)) is not type(value) or partition.get(key) != value:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_PARTITION_VALUE_MISMATCH:" + key)
    acquired = (
    ...
    if not acquired or not opened or not access_zero or not membership_unchanged:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_PRESERVATION_CONDITION_DERIVATION_BLOCKED")
...
def _default_read_safe_state(repository_root, commit: str, reader: Callable[[Any, str, str], bytes]) -> Mapping[str, Any]:
    return _strict_json_object(
        ...
        "V8F_T2_STATE_INVALID_JSON",
        "V8F_T2_STATE_DUPLICATE_KEY",
    )
...
def _default_read_safe_bridge(repository_root, commit: str, reader: Callable[[Any, str, str], bytes]) -> Mapping[str, Any]:
    return _strict_json_object(
        ...
        "V8F_T2_BRIDGE_INVALID_JSON",
        "V8F_T2_BRIDGE_DUPLICATE_KEY",
    )
...
def _resolve_t2_prefreeze_safe_evidence_with_dependencies(
    ...
    *,
    ...
    try:
        if git_blob_resolver(repository_root, V8F_REVIEWED_DESIGN_CANDIDATE_COMMIT, V8F_DESIGN_GIT_PATH) != V8F_DESIGN_CANDIDATE_BLOB_SHA:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_DESIGN_CANDIDATE_BLOB_MISMATCH")
        if git_blob_resolver(repository_root, reviewed_sha, V8F_DESIGN_GIT_PATH) != V8F_DESIGN_CANDIDATE_BLOB_SHA:
            raise V8FT2PrefreezePreservationBlocked("V8F_T2_CURRENT_DESIGN_BLOB_MISMATCH")
        expected_blobs = (
        ...
        terminal = _validate_v8e_terminal(
            _strict_text_bytes(
                ...
                "V8F_T2_V8E_TERMINAL_INVALID",
            )
        )
        historical_t2 = _validate_historical_v8e_t2_record(
            _strict_text_bytes(
                git_object_reader(repository_root, V8E_T2_PREFREEZE_COMMIT, V8E_T2_PREFREEZE_GIT_PATH),
                "V8F_T2_V8E_HISTORICAL_RECORD_INVALID",
            )
        )
        design = _validate_v8f_design(
            _strict_text_bytes(
                git_object_reader(repository_root, reviewed_sha, V8F_DESIGN_GIT_PATH),
                "V8F_T2_DESIGN_INVALID",
            )
        )
    except V8FT2PrefreezePreservationBlocked:
        raise
    except V8CGitProvenanceBlocked as error:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_SAFE_GIT_EVIDENCE_UNAVAILABLE") from error
...
def resolve_and_verify_t2_prefreeze_preservation(
    *,
    ...
    if branch != V8F_PRODUCTION_BRANCH:
        raise V8FT2PrefreezePreservationBlocked("V8F_T2_BRANCH_MISMATCH")
    head = _git_text(root, ["rev-parse", "HEAD"], "V8F_T2_HEAD_UNAVAILABLE")
...
__all__ = [
    ...
    "T2_SAFE_CONDITION_FIELDS",
    "V8E_T2_PREFREEZE_BLOB_SHA",
    "V8E_T2_PREFREEZE_COMMIT",
    "V8E_T2_PREFREEZE_GIT_PATH",
    ...
    "V8F_V8E_TERMINAL_RECORD_BLOB_SHA",
