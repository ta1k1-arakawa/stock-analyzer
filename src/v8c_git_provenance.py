"""V8C-specific Git provenance primitives.

`AI_RESEARCH_EXECUTION_RULES.md`, `V8C_HISTORICAL_RESEARCH_DESIGN_DRAFT.md`.
Every V8C production module must resolve "the verified current Git commit"
and read repository-fixed files from it through *this* module -- never
through `src.v8_partition.resolve_verified_production_git_commit()` (bound
to V8's own branch) or `src.v8b_git_provenance.resolve_verified_v8b_
production_git_commit()` (bound to V8B's own branch). Reusing either would
silently accept V8C production running from a checkout that only matches a
different study's branch.
...
strip list) and ``require_git_commit`` are re-exported unchanged (they
raise no repository-identity-bearing exception of their own that callers
need translated). ``resolve_git_blob``, ``read_git_object_bytes``, and
``require_strict_git_ancestor`` are wrapped, not re-exported directly:
the underlying V8B functions raise ``V8BGitProvenanceBlocked``, and every
V8C caller catches this module's own ``V8CGitProvenanceBlocked`` -- an
unwrapped re-export would silently escape every such ``except`` clause
throughout the V8C module set. This module reimplements only the
V8C-specific production branch/origin-identity binding, exactly mirroring
`src.v8b_git_provenance.resolve_verified_v8b_production_git_commit`'s own
logic and round-3 HIGH-1 remediation (an ``origin`` verified to resolve to
one of the ordinary HTTPS/SSH forms of exactly
``ta1k1-arakawa/stock-analyzer`` on ``github.com``, never a look-alike host
or a same-named branch in an unrelated repository).
...
PRODUCTION_BRANCH = "v8c-transport-resilience-implementation"
...
class V8CGitProvenanceBlocked(RuntimeError):
    """Fail-closed V8C Git provenance resolution/read error."""
...
def _run_git(args: list[str], *, repository_root) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise V8CGitProvenanceBlocked("GIT_PROVENANCE_SUBPROCESS_FAILED") from error
...
def _require_intended_github_repository_identity(repository_root) -> None:
    ...
    if result.returncode != 0:
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_ORIGIN_URL_UNAVAILABLE")
    if _canonical_github_owner_repo(result.stdout.strip()) != EXPECTED_GITHUB_OWNER_REPO:
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_ORIGIN_IDENTITY_MISMATCH")


def resolve_verified_v8c_production_git_commit(repository_root) -> str:
    """Resolve a clean checkout exactly matching V8C's own production branch ref.

    Deliberately performs no fetch. Requires ``origin`` to be verified as
    the intended ``ta1k1-arakawa/stock-analyzer`` GitHub repository identity
    before HEAD/origin-ref equality is even checked; V8's/V8B's own
    production branch refs are never consulted and cannot satisfy this
    check.
    """
    root = Path(repository_root)
    status = _run_git(["status", "--porcelain"], repository_root=root)
    if status.returncode != 0:
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_PROVENANCE_UNAVAILABLE")
    if status.stdout.strip():
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_WORKTREE_DIRTY")

    _require_intended_github_repository_identity(root)

    head = _run_git(["rev-parse", "HEAD"], repository_root=root)
    if head.returncode != 0:
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_HEAD_UNAVAILABLE")

    origin = _run_git(["rev-parse", "origin/" + PRODUCTION_BRANCH], repository_root=root)
    if origin.returncode != 0:
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_ORIGIN_REF_UNAVAILABLE")
    ...
    try:
        ...
        raise V8CGitProvenanceBlocked(error.reason) from error
    if head_commit != origin_commit:
        raise V8CGitProvenanceBlocked("PRODUCTION_GIT_HEAD_NOT_ORIGIN")
    return head_commit


def wrap_git_object_error(error: V8BGitProvenanceBlocked) -> V8CGitProvenanceBlocked:
    ...
    return V8CGitProvenanceBlocked(error.reason)
...
def resolve_git_blob(repository_root, commit: str, path: str) -> str:
    ...
    ``V8CGitProvenanceBlocked`` so every V8C caller's ``except
    V8CGitProvenanceBlocked`` clause actually catches it.
    """
    try:
