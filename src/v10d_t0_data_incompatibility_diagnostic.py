...
_INPUT_REASONS = frozenset(
    {
        ...
        "PAYLOAD_PATH_ESCAPE",
        "PAYLOAD_PATH_SYMLINK",
        "REQUIRED_FILE_UNAVAILABLE",
        "REQUIRED_FILE_UNSAFE",
        "CACHE_PAYLOAD_READ_INVALID",
        "CACHE_PAYLOAD_HASH_MISMATCH",
        "UNIVERSE_READ_INVALID",
        "UNIVERSE_IDENTITY_MISMATCH",
        "UNIVERSE_TICKER_IDENTITY_MISMATCH",
...
_PARSER_REASONS = frozenset(
    {
        ...
        "SPLIT_EVENT_SCHEMA_INVALID",
        "SPLIT_EVENT_RATIO_MISSING",
        "SPLIT_EVENT_RATIO_INVALID",
    }
)
_COMBINED_REASONS = frozenset(
    {
...
_CACHE_REASON_SETS = (
    ("INPUT_BYTE_OR_FILESET_CONTRACT", _INPUT_REASONS),
    ("PARSER_NORMALIZATION_CONTRACT", _PARSER_REASONS),
    ("COMBINED_SERIES_CONTRACT", _COMBINED_REASONS),
)
_CACHE_REASON_TO_STAGE = {
    reason: stage for stage, reasons in _CACHE_REASON_SETS for reason in reasons
}
if len(_CACHE_REASON_TO_STAGE) != sum(len(reasons) for _, reasons in _CACHE_REASON_SETS):
    raise RuntimeError("CACHE_REASON_MAPPING_OVERLAP")


def _stage_for_cache_reason(reason: object) -> str:
    return _CACHE_REASON_TO_STAGE.get(reason, "UNKNOWN_DATA_INCOMPATIBILITY")
