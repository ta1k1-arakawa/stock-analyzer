"""Exact-SHA verification of the frozen V8D authority bridges.
...
T1C_BRIDGE_PATH = "V8D_T1C_ALLOCATION_AUTHORITY_BRIDGE.json"
T1C_REVIEW_PATH = "V8D_T1C_ALLOCATION_AUTHORITY_BRIDGE_REVIEW.json"
T2_BRIDGE_PATH = "V8D_T2_AUTHORITY_BRIDGE.json"
T2_REVIEW_PATH = "V8D_T2_AUTHORITY_BRIDGE_REVIEW.json"

T1C_BRIDGE_SCHEMA = "V8D_T1C_ALLOCATION_AUTHORITY_BRIDGE_V1"
T2_BRIDGE_SCHEMA = "V8D_T2_AUTHORITY_BRIDGE_V1"
REVIEW_SCHEMA = "V8D_AUTHORITY_BRIDGE_INDEPENDENT_REVIEW_V1"
...
T1C_PRESERVATION_COMMIT = "58b70e2ce79c5f1195d1b8f20d348305e513c5f3"
T1C_PRESERVATION_BLOB = "049becb3d2743ef68dc278f424484919ba379cca"
...
T2_PRESERVATION_COMMIT = "8ae3032b42b426420f44c9f7194f0b1849c23e98"
T2_PRESERVATION_BLOB = "d023913b435ffd18eadef1e213c7ea43a49db331"
...
class V8DAuthorityBridgeBlocked(RuntimeError):
...
def _strict_json_object(raw: bytes, *, invalid_reason: str, duplicate_reason: str) -> dict[str, Any]:
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise V8DAuthorityBridgeBlocked(duplicate_reason)
            result[key] = value
        return result
    ...
    try:
        ...
        raise V8DAuthorityBridgeBlocked(invalid_reason) from error
    if not isinstance(value, dict):
        raise V8DAuthorityBridgeBlocked(invalid_reason)
    return value
...
def _read_git_json(repository_root, commit: str, path: str, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise V8DAuthorityBridgeBlocked(f"V8D_{label}_MISSING") from error
    return _strict_json_object(
        raw,
        invalid_reason=f"V8D_{label}_INVALID_JSON",
        duplicate_reason=f"V8D_{label}_DUPLICATE_KEY",
    )


def _require_exact_fields(document: dict[str, Any], fields: tuple[str, ...], reason: str) -> None:
    if set(document) != set(fields):
        raise V8DAuthorityBridgeBlocked(reason)
...
def _verify_review(
    ...
    _require_exact_fields(review, REVIEW_FIELDS, f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_SCHEMA_INVALID")
    if review["schema_version"] != REVIEW_SCHEMA:
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_SCHEMA_VERSION_MISMATCH")
    if review["study"] != STUDY:
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_STUDY_MISMATCH")
    if review["artifact_role"] != REVIEW_ROLE:
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_ROLE_MISMATCH")
    if review["logical_block"] != logical_block:
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_BLOCK_MISMATCH")
    if review["review_result"] != "PASS":
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_NOT_PASS")
    ...
    if not isinstance(reviewed_commit, str) or len(reviewed_commit) != 40 or any(
        ...
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_COMMIT_INVALID")
    ...
    if not isinstance(reviewed_blob, str) or len(reviewed_blob) != 40 or any(
        ...
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_BLOB_INVALID")
    try:
        resolved_blob = resolve_git_blob(repository_root, reviewed_commit, _bridge_path(logical_block))
    except V8DGitProvenanceBlocked as error:
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_MISSING") from error
    if resolved_blob != reviewed_blob:
        raise V8DAuthorityBridgeBlocked(f"V8D_{logical_block}_AUTHORITY_BRIDGE_REVIEW_BLOB_MISMATCH")
...
def _bridge_path(logical_block: str) -> str:
    ...
    raise V8DAuthorityBridgeBlocked("V8D_AUTHORITY_BRIDGE_LOGICAL_BLOCK_INVALID")


def _verify_t1c_bridge(bridge: dict[str, Any]) -> None:
    _require_exact_fields(bridge, T1C_BRIDGE_FIELDS, "V8D_T1C_AUTHORITY_BRIDGE_SCHEMA_INVALID")
    expected = {
        ...
        "human_gate": f"V8D_HUMAN_AUTHORIZE_T1C_AUTHORITY_BRIDGE_AT_{FROZEN_DESIGN_COMMIT}_FOR_{T1C_ALLOCATION_SELF_HASH}",
        "authorization_status": "AUTHORIZED",
    }
    for key, value in expected.items():
        if bridge[key] != value:
            raise V8DAuthorityBridgeBlocked(f"V8D_T1C_AUTHORITY_BRIDGE_{key.upper()}_MISMATCH")
    if not isinstance(bridge["authorization_note"], str) or not bridge["authorization_note"]:
        raise V8DAuthorityBridgeBlocked("V8D_T1C_AUTHORITY_BRIDGE_AUTHORIZATION_NOTE_INVALID")


def _verify_t2_bridge(bridge: dict[str, Any]) -> None:
    _require_exact_fields(bridge, T2_BRIDGE_FIELDS, "V8D_T2_AUTHORITY_BRIDGE_SCHEMA_INVALID")
    expected = {
        ...
        "preservation_recheck_result": "PASS",
        "human_gate": f"V8D_HUMAN_AUTHORIZE_T2_AUTHORITY_BRIDGE_AT_{FROZEN_DESIGN_COMMIT}_FOR_{T2_TICKER_LIST_SHA256}",
        "authorization_status": "AUTHORIZED",
    }
    for key, value in expected.items():
        if bridge[key] != value:
            raise V8DAuthorityBridgeBlocked(f"V8D_T2_AUTHORITY_BRIDGE_{key.upper()}_MISMATCH")
    if not isinstance(bridge["authorization_note"], str) or not bridge["authorization_note"]:
        raise V8DAuthorityBridgeBlocked("V8D_T2_AUTHORITY_BRIDGE_AUTHORIZATION_NOTE_INVALID")
...
def verify_stage_authority_bridge(repository_root, verified_head: str, logical_stage: str) -> dict[str, str]:
    ...
    if logical_stage == T1C_STAGE:
        ...
        raise V8DAuthorityBridgeBlocked("V8D_AUTHORITY_BRIDGE_STAGE_INVALID")
