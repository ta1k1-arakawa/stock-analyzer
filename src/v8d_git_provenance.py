"""V8D-specific Git provenance primitives.

`AI_RESEARCH_EXECUTION_RULES.md`, `V8D_TRANSPORT_AUDIT_SUCCESSOR_DESIGN_
DRAFT.md`. Every V8D production module must resolve "the verified current
...
resolver. Reusing either would silently accept V8D production running from
...
underlying V8B functions raise ``V8BGitProvenanceBlocked``, and every V8D
caller catches this module's own ``V8DGitProvenanceBlocked`` -- an
unwrapped re-export would silently escape every such ``except`` clause
throughout the V8D module set. This module reimplements only the
V8D-specific production branch/origin-identity binding, exactly mirroring
...
PRODUCTION_BRANCH = "v8d-transport-audit-design"
...
class V8DGitProvenanceBlocked(RuntimeError):
    """Fail-closed V8D Git provenance resolution/read error."""
...
def _run_git(args: list[str], *, repository_root) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise V8DGitProvenanceBlocked("GIT_PROVENANCE_SUBPROCESS_FAILED") from error
...
def _require_intended_github_repository_identity(repository_root) -> None:
    ...
    if result.returncode != 0:
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_ORIGIN_URL_UNAVAILABLE")
    if _canonical_github_owner_repo(result.stdout.strip()) != EXPECTED_GITHUB_OWNER_REPO:
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_ORIGIN_IDENTITY_MISMATCH")


def resolve_verified_v8d_production_git_commit(repository_root) -> str:
    """Resolve a clean checkout exactly matching V8D's own production branch ref.
    ...
    already-fetched ``origin/v8d-transport-audit-design`` state, on a clean
    ...
    if status.returncode != 0:
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_PROVENANCE_UNAVAILABLE")
    if status.stdout.strip():
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_WORKTREE_DIRTY")

    _require_intended_github_repository_identity(root)

    head = _run_git(["rev-parse", "HEAD"], repository_root=root)
    if head.returncode != 0:
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_HEAD_UNAVAILABLE")

    origin = _run_git(["rev-parse", "origin/" + PRODUCTION_BRANCH], repository_root=root)
    if origin.returncode != 0:
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_ORIGIN_REF_UNAVAILABLE")
    ...
    try:
        ...
        raise V8DGitProvenanceBlocked(error.reason) from error
    if head_commit != origin_commit:
        raise V8DGitProvenanceBlocked("PRODUCTION_GIT_HEAD_NOT_ORIGIN")
    return head_commit


def wrap_git_object_error(error: V8BGitProvenanceBlocked) -> V8DGitProvenanceBlocked:
    ...
    return V8DGitProvenanceBlocked(error.reason)
...
def resolve_git_blob(repository_root, commit: str, path: str) -> str:
    ...
    ``V8DGitProvenanceBlocked`` so every V8D caller's ``except
    V8DGitProvenanceBlocked`` clause actually catches it.
    """
    try:
