...
| `train.py` | AIモデルを学習して `models/stock_ai_model_8306.pkl` に保存 |
...
| 対象 | モデル | 売買ログ | Slack通知 |
| --- | --- | --- | --- |
| `target_stock`（8306） | `models/stock_ai_model_8306.pkl` | `data/trade_log_8306.csv` | あり |
| `log_only_stocks` | `models/stock_ai_model_<code>.pkl` | `data/trade_log_<code>.csv` | なし |

追加銘柄の `ai_params` を省略した場合は、`target_stock.ai_params` と同じ条件を引き継ぎます。銘柄ごとに条件を変えたい場合だけ、その銘柄の下へ `ai_params` を追加してください。

モデルファイルは実行のたびに全銘柄分を再学習して一時生成し、Gitには保存しません。8306の学習に失敗した場合は予測・通知を停止します。ログ専用銘柄の学習に失敗した場合は、その銘柄だけをスキップして8306とほかの銘柄は継続します。
